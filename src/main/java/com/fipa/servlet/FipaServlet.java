...
private JSONArray retCpflife(HttpServletRequest request) {
	 ...
	 logger.info(" ------> Inside checkretCpflife Method ");
		...
		try{
			...
			strRetAge = java.net.URLDecoder.decode(strRetAge,"UTF-8");
			strRetAge= strRetAge.replace("\'", "\''");
			
			
			String strRetPlanCode =  FipaUtils.getParamValue(request, "txtFldPlanCode");
			strRetPlanCode = java.net.URLDecoder.decode(strRetPlanCode,"UTF-8");
			strRetPlanCode= strRetPlanCode.replace("\'", "\''");
			
			String strPlegedvalue =  FipaUtils.getParamValue(request, "txtFldPlegedvalue");
			strPlegedvalue = java.net.URLDecoder.decode(strPlegedvalue,"UTF-8");
			strPlegedvalue= strPlegedvalue.replace("\'", "\''");
			
			String strTopRAErsvalue =  FipaUtils.getParamValue(request, "txtFldTopRAErsvalue");
			strTopRAErsvalue = java.net.URLDecoder.decode(strTopRAErsvalue,"UTF-8");
			strTopRAErsvalue= strTopRAErsvalue.replace("\'", "\''");
...
public JSONArray propertyDelete(HttpServletRequest request){
	...
	public JSONArray clientSearch(HttpServletRequest request){
		...
		try{
			...
			bothList = serv.clientSearch(dao,fpmsdao,strCustName,strCustNric,strLoggedAdvStfId,strLoggedStfType);
		...
		try{
			...
			if(clientTabSize > 0){
				...
				while(it.hasNext()){
					...
					System.out.println(strAnalysisFor);
...
//				System.out.println(clntDataJsnArr);
...
//				clientTabDetsList = serv.openClientProfile(dao,strFNAId);//SelfSpouse particulars
			...
			if(profilelen > 0){
				...
				System.out.println("advDeclareDetsTab--->"+advDeclareDetsTab);
				...
				System.out.println("typesOfAppDetsTab-->"+typesOfAppDetsTab);
				...
				System.out.println("fpmsfipaPolTab------->"+fpmsfipaPolTab);
...
//					System.out.println(cpfBalanceDetsTab);
...
//			 		sessMap.put("SKIN_COLOR",strSkinColor);
			 		...
			 		System.out.println(strSkinColor+"<-----strSkinColor");
...
//					
			...
			System.out.println("retValues->"+retValues);
	 ...
	 public JSONArray getFPMSlifeDets(HttpServletRequest request){
		 logger.info(" ------> Inside getFPMSlifeDets Method ");
			...
			try{
				...
				System.out.println("jsnObjPolTab-------->"+jsnObjPolTab);
...
//	  			}
	  		...
	  		retval.put(jsnClntFnaTabIdObj); 
	  		logger.info("fnaLifePremAmt retvalue-->"+retval);
	  		return retval;
	  	}
