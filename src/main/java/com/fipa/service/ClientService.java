...
//		 String strClientId = "";
		 ...
		 if(objMapping.containsKey(FipaConstant.SLFSPS_DETS)){
			 ...
			 jsnKeyObj.put("selfName", selfspsDets.getDfSelfName());
			 jsnKeyObj.put("selfNric", FipaUtils.nullOrBlank(selfspsDets.getDfSelfNric())? "" : selfspsDets.getDfSelfNric());
			 jsnKeyObj.put("fpmsCustId", FipaUtils.nullOrBlank(selfspsDets.getFpmsCustid())? "" : selfspsDets.getFpmsCustid());
		 }
...
//			 clientDB.saveLifeInsuranceDets(vectLifeInscDetails,strFNAId);
			 ...
			 if(objMapping.containsKey(FipaConstant.LIFEINSRCE_DETS)){
				 ...
				 Date expirydate = FipaUtils.nullOrBlank(request.getParameter("lipExpdate")) ? new Date() :FipaDateUtils.convertStringToDate(request.getParameter("lipExpdate")) ;
...
//	 } 
	...
	public List cpfPayoutIncomeSearch(DBInterfaceImpl dao,String strRetAge,
	 ...
	 public List searchPremAmtDetails(DBInterfaceImpl dto,DBInterfaceFpmsImpl fpmsdto, String strFNAId, String strClientAdvId, String strCustId){
		 ClientDB clientDB = new ClientDB();
	...
	public Vector 	getChildDetails(HttpServletRequest request) {
		...
		if(strArrChildMode != null && strArrChildMode.length>0){
			...
			for(int id=0;id<strArrChildMode.length;id++){
				...
				double dlbChldPerAnnEduCost = Double.parseDouble(FipaUtils.nullOrBlank(strArrChldPerAnnEduCost[id]) ? "0" :strArrChldPerAnnEduCost[id]);
				double dlbChldPerAnnLvCost = Double.parseDouble("0");
				double dlbChldAvailEdnFund = Double.parseDouble(FipaUtils.nullOrBlank(strArrChldAvailEdnFund[id]) ? "0" :strArrChldAvailEdnFund[id]);
			 
	//			Date crtdDate = FipaUtils.nullOrBlank(strArrChldCrtdDate[id]) ? new Date() :FipaDateUtils.convertStringToDate(strArrChldCrtdDate[id]) ;
				Date childdob =  FipaUtils.nullOrBlank(strArrChldDob[id]) ? null :FipaDateUtils.convertStringToDate(strArrChldDob[id]) ;
				...
				if(strArrChildMode[id].equalsIgnoreCase(FipaConstant.INSERT_MODE) && FipaUtils.nullOrBlank(strPKId)){  
					insChildList.add(new FnaChilddetails(null,null,strArrChldName[id],childdob,strArrChldAge[id],strArrChldRel[id],strArrChldGender[id],strArrChldYrs2ter[id],"","",strArrChldYrsofEdu[id],dlbChldPerAnnEduCost,dlbChldPerAnnLvCost,dlbChldAvailEdnFund,strArrChldRemrks[id],strLoggedUser,new Date(),null,null,"","",""));
					
				}
				
				if(strArrChildMode[id].equalsIgnoreCase(FipaConstant.UPDATE_MODE) || 
						(strArrChildMode[id].equalsIgnoreCase(FipaConstant.INSERT_MODE) && !FipaUtils.nullOrBlank(strPKId))){
					
					
					
					if(strPKId.startsWith("AS")){
						updChildListAS.add(new FnaChilddetails(strArrChldId[id],fnaselfspsDets,strArrChldName[id],childdob,strArrChldAge[id],strArrChldRel[id],strArrChldGender[id],strArrChldYrs2ter[id],"","",strArrChldYrsofEdu[id],dlbChldPerAnnEduCost,dlbChldPerAnnLvCost,dlbChldAvailEdnFund,strArrChldRemrks[id],strLoggedUser,new Date(),strLoggedUser,new Date(),"","",""));
					}else{
						Date crtdDate = FipaUtils.nullOrBlank(strArrChldCrtdDate[id]) ? new Date() :FipaDateUtils.convertStringToDate(strArrChldCrtdDate[id]) ;
						
						updChildList.add(new FnaChilddetails(strArrChldId[id],fnaselfspsDets,strArrChldName[id],childdob,strArrChldAge[id],strArrChldRel[id],strArrChldGender[id],strArrChldYrs2ter[id],"","",strArrChldYrsofEdu[id],dlbChldPerAnnEduCost,dlbChldPerAnnLvCost,dlbChldAvailEdnFund,strArrChldRemrks[id],strArrChldCrtdBy[id],crtdDate,strLoggedUser,new Date(),"","",""));
					}
				...
				if(strArrChildMode[id].equalsIgnoreCase(FipaConstant.DELETE_MODE)){
					...
					Date crtdDate = FipaUtils.nullOrBlank(strArrChldCrtdDate[id]) ? new Date() :FipaDateUtils.convertStringToDate(strArrChldCrtdDate[id]) ;
					 delChildList.add(new FnaChilddetails(strArrChldId[id],fnaselfspsDets,strArrChldName[id],childdob,strArrChldAge[id],strArrChldRel[id],strArrChldGender[id],strArrChldYrs2ter[id],"","",strArrChldYrsofEdu[id],dlbChldPerAnnEduCost,dlbChldPerAnnLvCost,dlbChldAvailEdnFund,strArrChldRemrks[id],strArrChldCrtdBy[id],crtdDate,strLoggedUser,new Date(),"","",""));
				}
			}
		}
		...
		vectChildDetails.add(updChildListAS);
			...
			logger.debug("Error in getChildDetails ->" , ex);
		}
	 	...
	 	try{
			...
			if(strArrDeptMode != null && strArrDeptMode.length>0){
				...
				for(int id=0;id<strArrDeptMode.length;id++){
					...
					double strDepnMonthctr = Double.parseDouble(FipaUtils.nullOrBlank(strArrDeptMthCont[id]) ? "0" :strArrDeptMthCont[id]);
					double strDepnProvSlf = Double.parseDouble(FipaUtils.nullOrBlank(strArrDeptSelfCont[id]) ? "0" :strArrDeptSelfCont[id]);
					double strDepnProvSps = Double.parseDouble(FipaUtils.nullOrBlank(strArrDeptSpsCont[id]) ? "0" :strArrDeptSpsCont[id]);
					
					String strPKId = FipaUtils.nullOrBlank(strArrDeptId[id])?"":strArrDeptId[id];
					
					Date crtdDate = FipaUtils.nullOrBlank(strArrDeptCrdtDate[id]) ? new Date() :FipaDateUtils.convertStringToDate(strArrDeptCrdtDate[id]) ;
					Date depndob =  FipaUtils.nullOrBlank(strArrDeptDob[id]) ? null :FipaDateUtils.convertStringToDate(strArrDeptDob[id]) ;
				
					FnaSelfspouseDets fnaselfspsDets = new FnaSelfspouseDets();				
					fnaselfspsDets.setFnaId(strFNAId);
					
					if(strArrDeptMode[id].equalsIgnoreCase(FipaConstant.INSERT_MODE) && FipaUtils.nullOrBlank(strPKId)){
						
						insDeptList.add(new FnaDependantDets(null,null,strArrDeptName[id],strArrDeptRel[id],depndob,strArrDeptAge[id],strArrDeptGender[id],strArrDeptYrToSup[id],strDepnMonthctr,strDepnProvSlf,strDepnProvSps,strLoggedUser,new Date(),null,null));
					 
					}
					
					if(strArrDeptMode[id].equalsIgnoreCase(FipaConstant.UPDATE_MODE)|| 
							(strArrDeptMode[id].equalsIgnoreCase(FipaConstant.INSERT_MODE) && !FipaUtils.nullOrBlank(strPKId))){
						
						if(strPKId.startsWith("AS")){
							updDeptListAS.add(new FnaDependantDets(strArrDeptId[id],fnaselfspsDets,strArrDeptName[id],strArrDeptRel[id],depndob,strArrDeptAge[id],strArrDeptGender[id],strArrDeptYrToSup[id],strDepnMonthctr,strDepnProvSlf,strDepnProvSps,strLoggedUser,new Date(),strLoggedUser,new Date()));
							 
						}else{
							updDeptList.add(new FnaDependantDets(strArrDeptId[id],fnaselfspsDets,strArrDeptName[id],strArrDeptRel[id],depndob,strArrDeptAge[id],strArrDeptGender[id],strArrDeptYrToSup[id],strDepnMonthctr,strDepnProvSlf,strDepnProvSps,strArrDeptCrdtBy[id],crtdDate,strLoggedUser,new Date()));
					...
					if(strArrDeptMode[id].equalsIgnoreCase(FipaConstant.DELETE_MODE)){
						delDeptList.add(new FnaDependantDets(strArrDeptId[id],fnaselfspsDets,strArrDeptName[id],strArrDeptRel[id],depndob,strArrDeptAge[id],strArrDeptGender[id],strArrDeptYrToSup[id],strDepnMonthctr,strDepnProvSlf,strDepnProvSps,strArrDeptCrdtBy[id],crtdDate,strLoggedUser,new Date()));
					}
				}
			}
			...
			logger.debug("Error in getDependentDetails ->" , ex);
		}
	...
	private Vector getCPFBalanceDets(HttpServletRequest request) {
		...
		for(MasterCpfAcctype cpfacc : lstMastAccTypeList){ 
			...
			if(FipaUtils.nullOrBlank(strCPFBalAccPkId) && (dblCPFBal >0)){
				insMthCntrbList.add(new FnaCpfBalanceDets(null, accType, null, dblCPFBal, "SELF", strLoggedUser, new Date(), null, null));
			}
			
			if(!FipaUtils.nullOrBlank(strCPFBalAccPkId)&& (dblCPFBal >0)){
				updMthCntrbList.add(new FnaCpfBalanceDets(strCPFBalAccPkId, accType, fnaSelfSpsDets, dblCPFBal, "SELF", strLoggedUser, new Date(), strLoggedUser, new Date()));
			}
			
			
			String strSpsCPFBalAccTypeId = FipaUtils.getParamValue(request, "hTxtFldSpsAccType"+cpfacc.getAccType());
			...
			double dblSpsCPFBal = Double.parseDouble(FipaUtils.nullOrBlank(strSpsCPFBalValue) ? "0" :strSpsCPFBalValue);
			if(FipaUtils.nullOrBlank(strSpsCPFBalAccPkId)&& (dblCPFBal >0)){
				insMthCntrbList.add(new FnaCpfBalanceDets(null, accTypeSps,null , dblSpsCPFBal, "SPOUSE", strLoggedUser, new Date(), null, null));
			}
			if(!FipaUtils.nullOrBlank(strSpsCPFBalAccPkId)&& (dblCPFBal >0)){
				updMthCntrbList.add(new FnaCpfBalanceDets(strSpsCPFBalAccPkId, accTypeSps,fnaSelfSps , dblSpsCPFBal, "SPOUSE", strLoggedUser, new Date(),strLoggedUser, new Date()));
			}
		} 
		...
		vectMthCntrbDetails.add(delMthCntrbList);
			...
			logger.debug("Error in getCPFBalanceDets ->" , ex);
		}
...
//			for(int id=0;id<strArrCCSlfOrSps.length;id++){
				...
				double dlbCCEmplrContrb = Double.parseDouble(FipaUtils.nullOrBlank(strEmplrContrb) ? "0" :strEmplrContrb);
				
				double dlbCCEmpleContrbSps = Double.parseDouble(FipaUtils.nullOrBlank(strEmpContrbSps) ? "0" :strEmpContrbSps); 
				double dlbCCEmplrContrbSps = Double.parseDouble(FipaUtils.nullOrBlank(strEmplrContrbSps) ? "0" :strEmplrContrbSps);
...
//				}else{
					...
					if(FipaUtils.nullOrBlank(strArrCCPkId) && (dlbCCEmpleContrb > 0 || dlbCCEmplrContrb > 0)){
						insMthCntrbList.add(new FnaCpfMonthcontDets(null,null,"Self",dlbCCEmpleContrb,dlbCCEmplrContrb,strLoggedUser,new Date(),null,null));
					}
					if(!FipaUtils.nullOrBlank(strArrCCPkId) && (dlbCCEmpleContrb > 0 || dlbCCEmplrContrb > 0) ){
						updMthCntrbList.add(new FnaCpfMonthcontDets(strArrCCPkId,fnaselfspsDets,"Self",dlbCCEmpleContrb,dlbCCEmplrContrb,strLoggedUser,new Date(),strLoggedUser,new Date()));
					}
					...
					if(FipaUtils.nullOrBlank(strArrCCPkIdSps) && (dlbCCEmpleContrbSps > 0 || dlbCCEmplrContrbSps > 0)){
						insMthCntrbList.add(new FnaCpfMonthcontDets(null,null,"Spouse",dlbCCEmpleContrbSps,dlbCCEmplrContrbSps,strLoggedUser,new Date(),null,null));
					}
					if(!FipaUtils.nullOrBlank(strArrCCPkIdSps) && (dlbCCEmpleContrbSps > 0 || dlbCCEmplrContrbSps > 0)){
						updMthCntrbList.add(new FnaCpfMonthcontDets(strArrCCPkIdSps,fnaselfsps,"Spouse",dlbCCEmpleContrbSps,dlbCCEmplrContrbSps,strLoggedUser,new Date(),strLoggedUser,new Date()));
					}
...
//			  }
		...
		vectMthCntrbDetails.add(delMthCntrbList); 
			...
			logger.debug("Error in getCPFMonthlyContrbDetails ->" , ex);
		}
	...
	private Vector getCPFTopUpDetails(HttpServletRequest request) {
		...
		if(strArrTpupMode != null && strArrTpupMode.length>0){
			...
			for(int id=0;id<strArrTpupMode.length;id++){
				...
				double dlbCtTopupAmt = Double.parseDouble(FipaUtils.nullOrBlank(strArrCtTopupAmt[id]) ? "0" :strArrCtTopupAmt[id]);
				 
				Date crtdDate = FipaUtils.nullOrBlank(strArrCtCrtdDate[id]) ? new Date() :FipaDateUtils.convertStringToDate(strArrCtCrtdDate[id]) ;
				...
				if(strArrTpupMode[id].equalsIgnoreCase(FipaConstant.INSERT_MODE) && FipaUtils.nullOrBlank(strPKId) ){
	//				insTopUpList.add(new FnaCpfTopupDets(null,fnaMasterCpftype,null,strArrCtType[id],strArrCtPaymentTerm[id],dlbCtTopupAmt,strArrCtIntendedYr[id],strLoggedUser,new Date(),null,null));
					 
				}
				
				if(strArrTpupMode[id].equalsIgnoreCase(FipaConstant.UPDATE_MODE)|| 
						(strArrTpupMode[id].equalsIgnoreCase(FipaConstant.INSERT_MODE) && !FipaUtils.nullOrBlank(strPKId))){
	//				updTopUpList.add(new FnaCpfTopupDets(strArrCtPkid[id],fnaMasterCpftype,fnaselfspsDets,strArrCtType[id],strArrCtPaymentTerm[id],dlbCtTopupAmt,strArrCtIntendedYr[id],strArrCtCrtdBy[id],crtdDate,strLoggedUser,new Date()));
					 
				}
				
				if(strArrTpupMode[id].equalsIgnoreCase(FipaConstant.DELETE_MODE)){
	//				delTopUpList.add(new FnaCpfTopupDets(strArrCtPkid[id],fnaMasterCpftype,fnaselfspsDets,strArrCtType[id],strArrCtPaymentTerm[id],dlbCtTopupAmt,strArrCtIntendedYr[id],strArrCtCrtdBy[id],crtdDate,strLoggedUser,new Date()));
				}
			}
		}
		...
		vectTopUpDetails.add(delTopUpList);
			...
			logger.debug("Error in getCPFTopUpDetails ->" , ex);
		}
...
//		sessMap = (HashMap<String, String>)session.getAttribute(FipaConstant.LOGGED_USER_INFO);
	...
	//	String[] strArrDedtMode    = FipaUtils.getParamArrValue(request, "txtFldCpfDedtMode");
	//	String[] strArrCdPkid      = FipaUtils.getParamArrValue(request, "txtFldCdPkid");
	//	String[] strArrCdApplicant = FipaUtils.getParamArrValue(request, "txtFldCdApplicant");
	//	String[] strArrCdDedtType  = FipaUtils.getParamArrValue(request, "txtFldCdDedtType");
	//	String[] strArrCdType      = FipaUtils.getParamArrValue(request, "txtFldCdType");
	//	String[] strArrCpfAcctype  = FipaUtils.getParamArrValue(request, "selmasterCpfAcctype");
	//	String[] strArrCdPayTerm   = FipaUtils.getParamArrValue(request, "txtFldCdPayTerm");
	//	String[] strArrCdDedtAmt   = FipaUtils.getParamArrValue(request, "txtFldCdDedtAmt");
	//	String[] strArrCdBalTerm   = FipaUtils.getParamArrValue(request, "txtFldCdBalTerm");
	//	String[] strArrCdCrtdBy    = FipaUtils.getParamArrValue(request, "txtFldCdCrtdBy"); 
	//	String[] strArrCdCrtdDate  = FipaUtils.getParamArrValue(request, "txtFldCdCrtdDate");
	...
	//			
	//			double dlbCdDedtAmt = Double.parseDouble(FipaUtils.nullOrBlank(strArrCdDedtAmt[id]) ? "0" :strArrCdDedtAmt[id]);
	//			short shtCdBalTerm = Short.parseShort(FipaUtils.nullOrBlank(strArrCdBalTerm[id]) ? "0" :strArrCdBalTerm[id]);
	//			
	//			Date crtdDate = FipaUtils.nullOrBlank(strArrCdCrtdDate[id]) ? new Date() :FipaDateUtils.convertStringToDate(strArrCdCrtdDate[id]) ;
	//			
	//
	//			MasterCpfAcctype fnaMasterCpftype = new MasterCpfAcctype();				
	...
	////				insDedtList.add(new FnaCpfDeductions(null,fnaMasterCpftype,null,strArrCdApplicant[id],strArrCdDedtType[id],strArrCdType[id],strArrCdPayTerm[id],dlbCdDedtAmt,shtCdBalTerm,null,null,null,null,null,strLoggedUser,new Date(),null,null));
	//				 
	//			}
	//			
	//			if(strArrDedtMode[id].equalsIgnoreCase(FipaConstant.UPDATE_MODE)){
	////				updDedtList.add(new FnaCpfDeductions(strArrCdPkid[id],fnaMasterCpftype,fnaselfspsDets,strArrCdApplicant[id],strArrCdDedtType[id],strArrCdType[id],strArrCdPayTerm[id],dlbCdDedtAmt,shtCdBalTerm,null,null,null,null,null,strArrCdCrtdBy[id],crtdDate,strLoggedUser,new Date()));
	//				 
	//			}
	//			
	//			if(strArrDedtMode[id].equalsIgnoreCase(FipaConstant.DELETE_MODE)){
	////				delDedtList.add(new FnaCpfDeductions(strArrCdPkid[id],fnaMasterCpftype,fnaselfspsDets,strArrCdApplicant[id],strArrCdDedtType[id],strArrCdType[id],strArrCdPayTerm[id],dlbCdDedtAmt,shtCdBalTerm,null,null,null,null,null,strArrCdCrtdBy[id],crtdDate,strLoggedUser,new Date()));
	//			}
	//		}
	//	}
	...
	//	vectDedtDetails.add(delDedtList);
			...
			logger.debug("Error in getCPFDedtnDetails ->" , ex);
		}
	...
	//	<!--changes done 19/06/2019 -->
		...
		if(strArrCADMode != null && strArrCADMode.length>0){
			...
			for(int id=0;id<strArrCADMode.length;id++){
				...
				double dlbCADAmt = Double.parseDouble(FipaUtils.nullOrBlank(strArrCADAmt[id]) ? "0" :strArrCADAmt[id]);
				double dlbBal=0; 
				
				Date perFromDate = FipaUtils.nullOrBlank(strArrCADPerFrom[id]) ? null :FipaDateUtils.convertStringToDate(strArrCADPerFrom[id]) ;
				Date perToDate = FipaUtils.nullOrBlank(strArrCADPerTo[id]) ? null:FipaDateUtils.convertStringToDate(strArrCADPerTo[id]) ;
				 
				Date crtdDate = FipaUtils.nullOrBlank(strArrCADCrtdDate[id]) ? new Date() :FipaDateUtils.convertStringToDate(strArrCADCrtdDate[id]) ;
	...
	//			<!--changes done 19/06/2019 -->
				if(strArrCADMode[id].equalsIgnoreCase(FipaConstant.INSERT_MODE) && FipaUtils.nullOrBlank(strPKId) ){
	
					insAddtDedtList.add(new FnaCpfDeductions(null,fnaMasterCpftype,null,strArrCADApplicant[id],strArrCADAplcnttype[id],"",strArrCADDedtType[id],perFromDate,perToDate,dlbCADAmt,strArrCADPayTerm[id],strArrCADType[id],dlbBal,strArrCADRetAccAge[id],"",strLoggedUser,new Date(),null,null,strArrCADRefId[id],strArrlifePlanName[id],strArrlifePolNo[id],strArrCADAgeFrom[id],strArrCADAgeTo[id],strArrCADYrsToRet[id],strArrCADChildName[id]));
				}
				
				if(strArrCADMode[id].equalsIgnoreCase(FipaConstant.UPDATE_MODE) || 
						(strArrCADMode[id].equalsIgnoreCase(FipaConstant.INSERT_MODE) && !FipaUtils.nullOrBlank(strPKId))){
					
					if(strPKId.startsWith("AS")){
						updAddtDedtListAS.add(new FnaCpfDeductions(strArrCADPkid[id],fnaMasterCpftype,fnaselfspsDets,strArrCADApplicant[id],strArrCADAplcnttype[id],"",strArrCADDedtType[id],perFromDate,perToDate,dlbCADAmt,strArrCADPayTerm[id],strArrCADType[id],dlbBal,strArrCADRetAccAge[id],"",strLoggedUser,new Date(),strLoggedUser,new Date(),strArrCADRefId[id],strArrlifePlanName[id],strArrlifePolNo[id],strArrCADAgeFrom[id],strArrCADAgeTo[id],strArrCADYrsToRet[id],strArrCADChildName[id]));
					}else{
						updAddtDedtList.add(new FnaCpfDeductions(strArrCADPkid[id],fnaMasterCpftype,fnaselfspsDets,strArrCADApplicant[id],strArrCADAplcnttype[id],"",strArrCADDedtType[id],perFromDate,perToDate,dlbCADAmt,strArrCADPayTerm[id],strArrCADType[id],dlbBal,strArrCADRetAccAge[id],"",strArrCADCrtdBy[id],crtdDate,strLoggedUser,new Date(),strArrCADRefId[id],strArrlifePlanName[id],strArrlifePolNo[id],strArrCADAgeFrom[id],strArrCADAgeTo[id],strArrCADYrsToRet[id],strArrCADChildName[id]));
				...
				if(strArrCADMode[id].equalsIgnoreCase(FipaConstant.DELETE_MODE)){ 
					delAddtDedtList.add(new FnaCpfDeductions(strArrCADPkid[id],fnaMasterCpftype,fnaselfspsDets,strArrCADApplicant[id],strArrCADAplcnttype[id],"",strArrCADDedtType[id],perFromDate,perToDate,dlbCADAmt,strArrCADPayTerm[id],strArrCADType[id],dlbBal,strArrCADRetAccAge[id],"",strArrCADCrtdBy[id],crtdDate,strLoggedUser,new Date(),strArrCADRefId[id],strArrlifePlanName[id],strArrlifePolNo[id],strArrCADAgeFrom[id],strArrCADAgeTo[id],strArrCADYrsToRet[id],strArrCADChildName[id]));
				}
			}
		}
		...
		vectAddtDedtDetails.add(updAddtDedtListAS);
			...
			logger.debug("Error in getCPFAddtnDedtnDetails ->" , ex);
		}
	...
	public Vector getFinGoalsDetails(HttpServletRequest request) {
		...
		List updFinGoalsList = new ArrayList();List updFinGoalsListAS = new ArrayList();
		...
		if(strArrFinGoalsMode != null && strArrFinGoalsMode.length>0){
			...
			for(int id=0;id<strArrFinGoalsMode.length;id++){
				...
				Date crtdDate = FipaUtils.nullOrBlank(strArrFGCrtdDate[id]) ? new Date() :FipaDateUtils.convertStringToDate(strArrFGCrtdDate[id]) ;
				...
				if(strArrFinGoalsMode[id].equalsIgnoreCase(FipaConstant.INSERT_MODE) && FipaUtils.nullOrBlank(strPKId)){
					insFinGoalsList.add(new FnaFingoalsconcern(null,null,strArrFinGoalType[id],strArrFinGoal[id],strArrFinGoalPrio[id],strLoggedUser,new Date(),null,null));
					 
				}
				
				if(strArrFinGoalsMode[id].equalsIgnoreCase(FipaConstant.UPDATE_MODE)|| 
						(strArrFinGoalsMode[id].equalsIgnoreCase(FipaConstant.INSERT_MODE) && !FipaUtils.nullOrBlank(strPKId))){
					if(strPKId.startsWith("AS")){
						updFinGoalsListAS.add(new FnaFingoalsconcern(strArrFnaGoalId[id],fnaselfspsDets,strArrFinGoalType[id],strArrFinGoal[id],strArrFinGoalPrio[id],strLoggedUser,new Date(),strLoggedUser,new Date()));
					}else{
						updFinGoalsList.add(new FnaFingoalsconcern(strArrFnaGoalId[id],fnaselfspsDets,strArrFinGoalType[id],strArrFinGoal[id],strArrFinGoalPrio[id],strArrFGCrtdBy[id],crtdDate,strLoggedUser,new Date()));	
					}
				...
				if(strArrFinGoalsMode[id].equalsIgnoreCase(FipaConstant.DELETE_MODE)){
					delFinGoalsList.add(new FnaFingoalsconcern(strArrFnaGoalId[id],fnaselfspsDets,strArrFinGoalType[id],strArrFinGoal[id],strArrFinGoalPrio[id],strArrFGCrtdBy[id],crtdDate,strLoggedUser,new Date()));
		...
		vectFinGoalsDetails.add(delFinGoalsList);vectFinGoalsDetails.add(updFinGoalsListAS);
		
		}catch(Exception ex){
			logger.debug("Error in getFinGoalsDetails ->" , ex);
		}
	...
	public Vector getOthAreaConrnDetails(HttpServletRequest request) {
		...
		List updOthConcnList = new ArrayList();		List updOthConcnListAS = new ArrayList();
		...
		if(strArrOthConcnMode != null && strArrOthConcnMode.length>0){
			...
			for(int id=0;id<strArrOthConcnMode.length;id++){
				...
				if(strArrOthConcnMode[id].equalsIgnoreCase(FipaConstant.INSERT_MODE) && FipaUtils.nullOrBlank(strPKId)){
					insOthConcnList.add(new FnaOthareaconcern(null,null,strArrOthAreaConcern[id],strLoggedUser,new Date(),null,null));
				}
				
				if(strArrOthConcnMode[id].equalsIgnoreCase(FipaConstant.UPDATE_MODE)|| 
						(strArrOthConcnMode[id].equalsIgnoreCase(FipaConstant.INSERT_MODE) && !FipaUtils.nullOrBlank(strPKId))){
					if(strPKId.startsWith("AS")){
						updOthConcnListAS.add(new FnaOthareaconcern(strArrFnaOacId[id],fnaselfspsDets,strArrOthAreaConcern[id],strLoggedUser,new Date(),strLoggedUser,new Date()));
					}else{
						updOthConcnList.add(new FnaOthareaconcern(strArrFnaOacId[id],fnaselfspsDets,strArrOthAreaConcern[id],strArrFgCrtdBy[id],crtdDate,strLoggedUser,new Date()));	
					}
					
				}
				
				if(strArrOthConcnMode[id].equalsIgnoreCase(FipaConstant.DELETE_MODE)){
					delOthConcnList.add(new FnaOthareaconcern(strArrFnaOacId[id],fnaselfspsDets,strArrOthAreaConcern[id],strArrFgCrtdBy[id],crtdDate,strLoggedUser,new Date()));
				}
			}
		}
		...
		vectOthConcnDetails.add(delOthConcnList);vectOthConcnDetails.add(updOthConcnListAS);
		}catch(Exception ex){
			logger.debug("Error in getOthAreaConrnDetails ->" , ex);
		}
	...
	private Vector getReasonsDetails(HttpServletRequest request) {
		...
		List updResonList = new ArrayList();List updResonListAS = new ArrayList();
		...
		if(strArrReasonMode != null && strArrReasonMode.length>0){
			...
			for(int id=0;id<strArrReasonMode.length;id++){
				...
				Date crtdDate = FipaUtils.nullOrBlank(strArrRsnCrtdDate[id]) ? new Date() :FipaDateUtils.convertStringToDate(strArrRsnCrtdDate[id]) ;
				...
				if(strArrReasonMode[id].equalsIgnoreCase(FipaConstant.INSERT_MODE) && FipaUtils.nullOrBlank(strPKId)){
					insResonList.add(new FnaRecomReasons(null,null,strArrReason[id],strLoggedUser,new Date(),null,null));
				 
				}
				
				if(strArrReasonMode[id].equalsIgnoreCase(FipaConstant.UPDATE_MODE) || 
						(strArrReasonMode[id].equalsIgnoreCase(FipaConstant.INSERT_MODE) && !FipaUtils.nullOrBlank(strPKId))){
					if(strPKId.startsWith("AS")){
						updResonListAS.add(new FnaRecomReasons(strArrReasonId[id],fnaselfspsDets,strArrReason[id],strLoggedUser,new Date(),strLoggedUser,new Date()));
					}else{
						updResonList.add(new FnaRecomReasons(strArrReasonId[id],fnaselfspsDets,strArrReason[id],strArrRsnCrtdBy[id],crtdDate,strLoggedUser,new Date()));	
					}
				...
				if(strArrReasonMode[id].equalsIgnoreCase(FipaConstant.DELETE_MODE)){
					delResonList.add(new FnaRecomReasons(strArrReasonId[id],fnaselfspsDets,strArrReason[id],strArrRsnCrtdBy[id],crtdDate,strLoggedUser,new Date()));
		...
		vectResonDetails.add(delResonList);vectResonDetails.add(updResonListAS);
		
		}catch(Exception ex){
			logger.debug("Error in getReasonsDetails ->" , ex);
		}
	...
	public Vector getRetCpfLifeDetails(HttpServletRequest request) {
		...
		List updCpfLifeList = new ArrayList();List updCpfLifeListAS = new ArrayList();
		...
		if(strArrCpfLifeMode != null && strArrCpfLifeMode.length>0){
			...
			for(int id=0;id<strArrCpfLifeMode.length;id++){
				...
				double dlbCPFLifePremAmt = Double.parseDouble(FipaUtils.nullOrBlank(strArrCpfLifePrem[id]) ? "0" :strArrCpfLifePrem[id]);
				double dlbCPFLifeInc = Double.parseDouble(FipaUtils.nullOrBlank(strArrCpfLifePayoutInc[id]) ? "0" :strArrCpfLifePayoutInc[id]);
				double dlbCPFLifeFdTrans = Double.parseDouble(FipaUtils.nullOrBlank(strArrCpfLifeFdTrans[id]) ? "0" :strArrCpfLifeFdTrans[id]);
				...
				if(strArrCpfLifeMode[id].equalsIgnoreCase(FipaConstant.INSERT_MODE)  && FipaUtils.nullOrBlank(strPKId) ){
					insCpfLifeList.add(new FnaRetireplanCpflife(null,null,strArrCpfLifeAge[id],strArrCpfLifeTypes[id],dlbCPFLifePremAmt,dlbCPFLifeInc,dlbCPFLifeFdTrans));
				 
				}
				
				if(strArrCpfLifeMode[id].equalsIgnoreCase(FipaConstant.UPDATE_MODE) || 
						(strArrCpfLifeMode[id].equalsIgnoreCase(FipaConstant.INSERT_MODE) && !FipaUtils.nullOrBlank(strPKId))){
					if(strPKId.startsWith("AS")){
						updCpfLifeListAS.add(new FnaRetireplanCpflife(strArrCpfLifeId[id],fnaselfspsDets,strArrCpfLifeAge[id],strArrCpfLifeTypes[id],dlbCPFLifePremAmt,dlbCPFLifeInc,dlbCPFLifeFdTrans));
					}else{
						updCpfLifeList.add(new FnaRetireplanCpflife(strArrCpfLifeId[id],fnaselfspsDets,strArrCpfLifeAge[id],strArrCpfLifeTypes[id],dlbCPFLifePremAmt,dlbCPFLifeInc,dlbCPFLifeFdTrans));	
					}
				...
				if(strArrCpfLifeMode[id].equalsIgnoreCase(FipaConstant.DELETE_MODE)){
					delCpfLifeList.add(new FnaRetireplanCpflife(strArrCpfLifeId[id],fnaselfspsDets,strArrCpfLifeAge[id],strArrCpfLifeTypes[id],dlbCPFLifePremAmt,dlbCPFLifeInc,dlbCPFLifeFdTrans));
		...
		vectCpfLifeDetails.add(delCpfLifeList);vectCpfLifeDetails.add(updCpfLifeListAS);
		
		}catch(Exception ex){
			logger.debug("Error in getRetCpfLifeDetails ->" , ex);
		}
	...
	private Vector getAutoAlterDetails(HttpServletRequest request) {
		...
		List updAtoAltList = new ArrayList(); 	List updAtoAltListAS = new ArrayList(); 
		...
		if(strArrAtoAltMode != null && strArrAtoAltMode.length>0){
			...
			for(int id=0;id<strArrAtoAltMode.length;id++){
				...
				Date crtdDate = FipaUtils.nullOrBlank(strArrAtoAltCrtdDate[id]) ? new Date() :FipaDateUtils.convertStringToDate(strArrAtoAltCrtdDate[id]) ;
				 
				String strPKId = FipaUtils.nullOrBlank(strArrAtoAltId[id])?"":strArrAtoAltId[id];
				
				
				if(strArrAtoAltMode[id].equalsIgnoreCase(FipaConstant.INSERT_MODE) && FipaUtils.nullOrBlank(strPKId)){
					insAtoAltList.add(new FnaAutoAlter(null,null,strArrAtoAltScrName[id],strArrAtoAltAttrName[id],strArrAtoAltOrgName[id],strArrAtoAltNewName[id],strLoggedUser,new Date()));
				 
				}
				
				if(strArrAtoAltMode[id].equalsIgnoreCase(FipaConstant.UPDATE_MODE) || 
						(strArrAtoAltMode[id].equalsIgnoreCase(FipaConstant.INSERT_MODE) && !FipaUtils.nullOrBlank(strPKId))){
					if(strPKId.startsWith("AS")){
						updAtoAltListAS.add(new FnaAutoAlter(strArrAtoAltId[id],strFNAId,strArrAtoAltScrName[id],strArrAtoAltAttrName[id],strArrAtoAltOrgName[id],strArrAtoAltNewName[id],strLoggedUser,new Date()));
						 
					}else{
						updAtoAltList.add(new FnaAutoAlter(strArrAtoAltId[id],strFNAId,strArrAtoAltScrName[id],strArrAtoAltAttrName[id],strArrAtoAltOrgName[id],strArrAtoAltNewName[id],strArrAtoAltCrtdBy[id],crtdDate));
		...
		vectAtoAltDets.add(updAtoAltListAS); 
			...
			logger.debug("Error in getAutoAlterDetails ->" , ex);
		}
	...
	private Vector getAnalysisTypes(HttpServletRequest request) {
		...
		String strLoggedUser =(String)session.getAttribute(FipaConstant.LOGGED_USER_ID);
		 ...
		 if(strArrAnalysis != null && strArrAnalysis.length>0){
			...
			for(int id=0;id<strArrAnalysis.length;id++){
				...
				if(!FipaUtils.nullObj(strArrAppTypeId[id]) && !FipaUtils.nullOrBlank(strArrAnalysis[id]) && !FipaUtils.nullOrBlank(strFNAId)){
					delFinGoalsList.add(new FnaApptypes(strArrAppTypeId[id],fnaAnalysis,fnaselfspsDets,strLoggedUser,new Date()));
					insFinGoalsList.add(new FnaApptypes(null,fnaAnalysis,fnaselfspsDets,strLoggedUser,new Date()));
				}else{
					insFinGoalsList.add(new FnaApptypes(null,fnaAnalysis,fnaselfspsDets,strLoggedUser,new Date()));	
				}
		...
		vectAnalysisType.add(delFinGoalsList);
			...
			logger.debug("Error in getAnalysisTypes ->" , ex);
		}
	...
	public Vector getSavingsInvDetails(HttpServletRequest request) {
		...
		List updSAInvList = new ArrayList();		List updSAInvListAS = new ArrayList();
		...
		if(strArrSAInvMode != null && strArrSAInvMode.length>0){
			...
			for(int id=0;id<strArrSAInvMode.length;id++){
				...
				double dblSAInvAmt = Double.parseDouble(FipaUtils.nullOrBlank(strArrSAInvAmt[id]) ? "0" :strArrSAInvAmt[id]);
				 
				double dblSAInvWhen = Double.parseDouble(FipaUtils.nullOrBlank(strArrSAInvWhen[id]) ? "0" :strArrSAInvWhen[id]);
				
				Date crtdDate = FipaUtils.nullOrBlank(strArrSAInvCrdtDate[id]) ? new Date() :FipaDateUtils.convertStringToDate(strArrSAInvCrdtDate[id]) ;
				...
				if(strArrSAInvMode[id].equalsIgnoreCase(FipaConstant.INSERT_MODE) && FipaUtils.nullOrBlank(strPKId)){
					insSAInvList.add(new FnaSavingsinvDets(null,null,strArrSAInvPurpose[id],dblSAInvWhen,dblSAInvAmt,strArrselSAPriority[id],strArrselSARiskLvl[id],strLoggedUser,new Date(),null,null));
				 
				}
				
				if(strArrSAInvMode[id].equalsIgnoreCase(FipaConstant.UPDATE_MODE)|| 
						(strArrSAInvMode[id].equalsIgnoreCase(FipaConstant.INSERT_MODE) && !FipaUtils.nullOrBlank(strPKId))){
					if(strPKId.startsWith("AS")){
						updSAInvListAS.add(new FnaSavingsinvDets(strArrSAInvId[id],fnaselfspsDets,strArrSAInvPurpose[id],dblSAInvWhen,dblSAInvAmt,strArrselSAPriority[id],strArrselSARiskLvl[id],strLoggedUser,new Date(),strLoggedUser,new Date()));
					}else{
						updSAInvList.add(new FnaSavingsinvDets(strArrSAInvId[id],fnaselfspsDets,strArrSAInvPurpose[id],dblSAInvWhen,dblSAInvAmt,strArrselSAPriority[id],strArrselSARiskLvl[id],strArrSAInvCrdtBy[id],crtdDate,strLoggedUser,new Date()));	
					}
				...
				if(strArrSAInvMode[id].equalsIgnoreCase(FipaConstant.DELETE_MODE)){
					delSAInvList.add(new FnaSavingsinvDets(strArrSAInvId[id],fnaselfspsDets,strArrSAInvPurpose[id],dblSAInvWhen,dblSAInvAmt,strArrselSAPriority[id],strArrselSARiskLvl[id],strArrSAInvCrdtBy[id],crtdDate,strLoggedUser,new Date()));
		...
		vectSAInvDetails.add(delSAInvList);vectSAInvDetails.add(updSAInvListAS);
		}catch(Exception ex){
			logger.debug("Error in getSavingsInvDetails ->" , ex);
		}
	...
	public Vector getVehOwnDetails(HttpServletRequest request) {
		...
		List updVehList = new ArrayList();	List updVehListAS = new ArrayList();
	 ...
	 try{
		...
		if(strArrVehMode != null && strArrVehMode.length>0){
			...
			for(int id=0;id<strArrVehMode.length;id++){
				...
				double dblVehMktVal = Double.parseDouble(FipaUtils.nullOrBlank(strArrVehMktVal[id]) ? "0" :strArrVehMktVal[id]);
				double dblLnVal = Double.parseDouble(FipaUtils.nullOrBlank(strArrVehLnVal[id]) ? "0" :strArrVehLnVal[id]);
				double dblMthInstl = Double.parseDouble(FipaUtils.nullOrBlank(strArrVehMthInstl[id]) ? "0" :strArrVehMthInstl[id]);
				double dlbVehPerd = Double.parseDouble(FipaUtils.nullOrBlank(strArrVehPerd[id]) ? "0" :strArrVehPerd[id]);
				
				Date crtdDate = FipaUtils.nullOrBlank(strArrVehCrtdDate[id]) ? new Date() :FipaDateUtils.convertStringToDate(strArrVehCrtdDate[id]) ;
				...
				if(strArrVehMode[id].equalsIgnoreCase(FipaConstant.INSERT_MODE) && FipaUtils.nullOrBlank(strPKId)){
					insVehList.add(new FnaVehicleownDets(null,null,strArrVehOwner[id],dblVehMktVal,dblLnVal,strArrVehLnBank[id],dlbVehPerd,dblMthInstl,strArrVehSoldonDth[id],strArrVehRmk[id],strLoggedUser,new Date()));
					 
				}
				
				if(strArrVehMode[id].equalsIgnoreCase(FipaConstant.UPDATE_MODE) || 
						(strArrVehMode[id].equalsIgnoreCase(FipaConstant.INSERT_MODE) && !FipaUtils.nullOrBlank(strPKId))){
					if(strPKId.startsWith("AS")){
						updVehListAS.add(new FnaVehicleownDets(strArrVehId[id],fnaselfspsDets,strArrVehOwner[id],dblVehMktVal,dblLnVal,strArrVehLnBank[id],dlbVehPerd,dblMthInstl,strArrVehSoldonDth[id],strArrVehRmk[id],strLoggedUser,new Date()));
						
					}else{
						updVehList.add(new FnaVehicleownDets(strArrVehId[id],fnaselfspsDets,strArrVehOwner[id],dblVehMktVal,dblLnVal,strArrVehLnBank[id],dlbVehPerd,dblMthInstl,strArrVehSoldonDth[id],strArrVehRmk[id],strArrVehCrtdBy[id],crtdDate));
				...
				if(strArrVehMode[id].equalsIgnoreCase(FipaConstant.DELETE_MODE)){
					delVehList.add(new FnaVehicleownDets(strArrVehId[id],fnaselfspsDets,strArrVehOwner[id],dblVehMktVal,dblLnVal,strArrVehLnBank[id],dlbVehPerd,dblMthInstl,strArrVehSoldonDth[id],strArrVehRmk[id],strArrVehCrtdBy[id],crtdDate));
				}
			}
		}
		...
		vectVehDetails.add(delVehList);vectVehDetails.add(updVehListAS);
	 }catch(Exception ex){
			logger.debug("Error in getVehOwnDetails ->" , ex);
		}
	...
	public Vector getPropOwnDetails(HttpServletRequest request) {
		...
		List updPropList = new ArrayList();List updPropListAS = new ArrayList();
	...
	//	String[] strArrPropCpfYrstoPay  = FipaUtils.getParamArrValue(request, "txtFldPropCpfYrstoPay"); 
		...
		if(strArrPropCpfMode != null && strArrPropCpfMode.length>0){
			...
			for(int id=0;id<strArrPropCpfMode.length;id++){
				...
				double dblPropCpfMktVal = Double.parseDouble(FipaUtils.nullOrBlank(strArrPropCpfMktVal[id]) ? "0" :strArrPropCpfMktVal[id]);
				double dblPropCpfMortageos = Double.parseDouble(FipaUtils.nullOrBlank(strArrPropCpfMortageos[id]) ? "0" :strArrPropCpfMortageos[id]);
				double dblPropCpfApprecrate = Double.parseDouble(FipaUtils.nullOrBlank(strArrPropCpfApprecrate[id]) ? "0" :strArrPropCpfApprecrate[id]);
				double dblPropCpfOsloanPerd = Double.parseDouble(FipaUtils.nullOrBlank(strArrPropCpfOsloanPerd[id]) ? "0" :strArrPropCpfOsloanPerd[id]);
				double dblPropCpfLoanCash = 0;
				double dblPropCpfYrstoPay = 0;
				double dblPropCurRetInc = Double.parseDouble(FipaUtils.nullOrBlank(strArrPropCurRetInc[id]) ? "0" :strArrPropCurRetInc[id]);
				double dblPropYrsToRent = Double.parseDouble(FipaUtils.nullOrBlank(strArrPropYrsToRent[id]) ? "0" :strArrPropYrsToRent[id]);
	  
				
				
				double dblPropCurAnlRetInc=Double.parseDouble(FipaUtils.nullOrBlank(strArrPropCurAnlRetInc[id]) ? "0" :strArrPropCurAnlRetInc[id]);
				double dblPropCpfLnBySlfCpf = Double.parseDouble(FipaUtils.nullOrBlank(strArrPropCpfLnBySlfCpf[id]) ? "0" :strArrPropCpfLnBySlfCpf[id]);
				double dblPropCpfLnBySlfCash = Double.parseDouble(FipaUtils.nullOrBlank(strArrPropCpfLnBySlfCash[id]) ? "0" :strArrPropCpfLnBySlfCash[id]);
				double dblPropCpfLnBySpsCpf = Double.parseDouble(FipaUtils.nullOrBlank(strArrPropCpfLnBySpsCpf[id]) ? "0" :strArrPropCpfLnBySpsCpf[id]);
				double dblPropCpfLnBySpsCash = Double.parseDouble(FipaUtils.nullOrBlank(strArrPropCpfLnBySpsCash[id]) ? "0" :strArrPropCpfLnBySpsCash[id]);
				double dblPropLoancpf=0;
				double dblPropMonthRentincome=0;
				double dblPropFvOnRent= Double.parseDouble(FipaUtils.nullOrBlank(strArrPropFvOnRent[id]) ? "0" :strArrPropFvOnRent[id]);
				
				
				double dblPropRentPerd= Double.parseDouble(FipaUtils.nullOrBlank(strArrPropRentPerd[id]) ? "0" :strArrPropRentPerd[id]);
				
				Date crtdDate = FipaUtils.nullOrBlank(strArrPropCpfCrtdDate[id]) ? new Date() :FipaDateUtils.convertStringToDate(strArrPropCpfCrtdDate[id]) ;
				
				FnaSelfspouseDets fnaselfspsDets = new FnaSelfspouseDets();				
				fnaselfspsDets.setFnaId(strFNAId);
				
				String strPKId = FipaUtils.nullOrBlank(strArrPropCpfOwnId[id])?"":strArrPropCpfOwnId[id];
//				System.out.println(strPKId +"---------------------"+strArrPropCpfMode[id]);
				
				if(strArrPropCpfMode[id].equalsIgnoreCase(FipaConstant.INSERT_MODE) && FipaUtils.nullOrBlank(strPKId)){
					insPropList.add(new FnaPropownDets(null,null, "CPF",strArrPropCpfOwnDesc[id],strArrPropCpfOwnership[id],dblPropCpfMktVal,dblPropCpfMortageos,strArrPropCpfObj[id], dblPropCpfApprecrate,dblPropCpfOsloanPerd,dblPropCpfLnBySlfCash,dblPropCpfLnBySpsCash,dblPropCpfLnBySlfCpf,dblPropCpfLnBySpsCpf,dblPropMonthRentincome,dblPropCpfYrstoPay, strArrPropCpfSold[id],strArrPropRentOnRet[id],dblPropCpfLoanCash,dblPropLoancpf,dblPropCurRetInc,dblPropCurAnlRetInc,dblPropYrsToRent,strLoggedUser,new Date(), null, null,strArrPropRefId[id],dblPropFvOnRent,dblPropRentPerd));
				                                  	
				}	
				
				if(strArrPropCpfMode[id].equalsIgnoreCase(FipaConstant.UPDATE_MODE)|| 
						(strArrPropCpfMode[id].equalsIgnoreCase(FipaConstant.INSERT_MODE) && !FipaUtils.nullOrBlank(strPKId))){
					
					if(strPKId.startsWith("AS")){
						updPropListAS.add(new FnaPropownDets(strArrPropCpfOwnId[id],fnaselfspsDets ,"CPF",strArrPropCpfOwnDesc[id],strArrPropCpfOwnership[id],dblPropCpfMktVal,dblPropCpfMortageos,strArrPropCpfObj[id], dblPropCpfApprecrate,dblPropCpfOsloanPerd,dblPropCpfLnBySlfCash,dblPropCpfLnBySpsCash,dblPropCpfLnBySlfCpf,dblPropCpfLnBySpsCpf,dblPropMonthRentincome,dblPropCpfYrstoPay, strArrPropCpfSold[id],strArrPropRentOnRet[id],dblPropCpfLoanCash,dblPropLoancpf,dblPropCurRetInc,dblPropCurAnlRetInc,dblPropYrsToRent,strLoggedUser,new Date(),strLoggedUser,new Date(),strArrPropRefId[id],dblPropFvOnRent,dblPropRentPerd));
						 
					}else{
						updPropList.add(new FnaPropownDets(strArrPropCpfOwnId[id],fnaselfspsDets ,"CPF",strArrPropCpfOwnDesc[id],strArrPropCpfOwnership[id],dblPropCpfMktVal,dblPropCpfMortageos,strArrPropCpfObj[id], dblPropCpfApprecrate,dblPropCpfOsloanPerd,dblPropCpfLnBySlfCash,dblPropCpfLnBySpsCash,dblPropCpfLnBySlfCpf,dblPropCpfLnBySpsCpf,dblPropMonthRentincome,dblPropCpfYrstoPay, strArrPropCpfSold[id],strArrPropRentOnRet[id],dblPropCpfLoanCash,dblPropLoancpf,dblPropCurRetInc,dblPropCurAnlRetInc,dblPropYrsToRent,strArrPropCpfCrtdBy[id],crtdDate,strLoggedUser,new Date(),strArrPropRefId[id],dblPropFvOnRent,dblPropRentPerd));
				...
				if(strArrPropCpfMode[id].equalsIgnoreCase(FipaConstant.DELETE_MODE)){
					delPropList.add(new FnaPropownDets(strArrPropCpfOwnId[id],fnaselfspsDets,"CPF",strArrPropCpfOwnDesc[id],strArrPropCpfOwnership[id],dblPropCpfMktVal,dblPropCpfMortageos,strArrPropCpfObj[id], dblPropCpfApprecrate,dblPropCpfOsloanPerd,dblPropCpfLnBySlfCash,dblPropCpfLnBySpsCash,dblPropCpfLnBySlfCpf,dblPropCpfLnBySpsCpf,dblPropMonthRentincome,dblPropCpfYrstoPay, strArrPropCpfSold[id],strArrPropRentOnRet[id],dblPropCpfLoanCash,dblPropLoancpf,dblPropCurRetInc,dblPropCurAnlRetInc,dblPropYrsToRent,strArrPropCpfCrtdBy[id],crtdDate,strLoggedUser,new Date(),strArrPropRefId[id],dblPropFvOnRent,dblPropRentPerd));
		...
		vectPropDetails.add(delPropList);vectPropDetails.add(updPropListAS);
		 }catch(Exception ex){
				logger.debug("Error in getPropOwnDetails ->" , ex);
			}
	...
	public Vector getAssetDetails(HttpServletRequest request) {
		...
		List updAsstList = new ArrayList();List updAsstListAS = new ArrayList();
...
//		String[] strArrBuisBusiPerId    = FipaUtils.getParamArrValue(request, "txtFldBuisBusiPersId");
		...
		for(int id=0;id<strArrPerAsstMode.length;id++){
			...
			double dblPerEstApprValue = Double.parseDouble(FipaUtils.nullOrBlank(strArrPerEstApprValue[id]) ? "0" :strArrPerEstApprValue[id]);
			double dblPerPurInvValue = Double.parseDouble(FipaUtils.nullOrBlank(strArrPerPurInvValue[id]) ? "0" :strArrPerPurInvValue[id]);
			double dblPerCurrValue = Double.parseDouble(FipaUtils.nullOrBlank(strArrPerCurrValue[id]) ? "0" :strArrPerCurrValue[id]);
			double dblPerOsValue = Double.parseDouble(FipaUtils.nullOrBlank(strArrPerOsValue[id]) ? "0" :strArrPerOsValue[id]);
			    
			double  dblPerChildEdnPrcnt = Double.parseDouble(FipaUtils.nullOrBlank(strArrPerChildEdnPrcnt[id]) ? "0" :strArrPerChildEdnPrcnt[id]);
			double  dblPerRetPlanPrcnt = Double.parseDouble(FipaUtils.nullOrBlank(strArrPerRetPlanPrcnt[id]) ? "0" :strArrPerRetPlanPrcnt[id]);
			
			Date crtdDate = FipaUtils.nullOrBlank(strArrPerCrtdDate[id]) ? new Date() :FipaDateUtils.convertStringToDate(strArrPerCrtdDate[id]) ;
			...
			if(strArrPerAsstMode[id].equalsIgnoreCase(FipaConstant.INSERT_MODE) && FipaUtils.nullOrBlank(strPKId)){
				insAsstList.add(new FnaAssetBusiandpersdets(null,null,"PERSONAL",strArrPerAcctHolder[id],strArrPerTypeOfAsset[id],strArrPerNameOfAsset[id],dblPerPurInvValue,dblPerCurrValue,dblPerOsValue,dblPerEstApprValue,strArrPerYrs2keep[id],dblPerChildEdnPrcnt,dblPerRetPlanPrcnt,strArrPerRemarks[id],strLoggedUser,new Date(),null,null));
	 
			}
			
			if(strArrPerAsstMode[id].equalsIgnoreCase(FipaConstant.UPDATE_MODE)  || 
					(strArrPerAsstMode[id].equalsIgnoreCase(FipaConstant.INSERT_MODE) && !FipaUtils.nullOrBlank(strPKId))){
				
				if(strPKId.startsWith("AS")){
					updAsstListAS.add(new FnaAssetBusiandpersdets(strPKId,fnaselfspsDets,"PERSONAL",strArrPerAcctHolder[id],strArrPerTypeOfAsset[id],strArrPerNameOfAsset[id],dblPerPurInvValue,dblPerCurrValue,dblPerOsValue,dblPerEstApprValue,strArrPerYrs2keep[id],dblPerChildEdnPrcnt,dblPerRetPlanPrcnt,strArrPerRemarks[id],strLoggedUser,new Date(),strLoggedUser,new Date()));
					 
				}else{
					updAsstList.add(new FnaAssetBusiandpersdets(strPKId,fnaselfspsDets,"PERSONAL",strArrPerAcctHolder[id],strArrPerTypeOfAsset[id],strArrPerNameOfAsset[id],dblPerPurInvValue,dblPerCurrValue,dblPerOsValue,dblPerEstApprValue,strArrPerYrs2keep[id],dblPerChildEdnPrcnt,dblPerRetPlanPrcnt,strArrPerRemarks[id],strArrPerCrtdBy[id],crtdDate,strLoggedUser,new Date()));
			...
			if(strArrPerAsstMode[id].equalsIgnoreCase(FipaConstant.DELETE_MODE)){
				delAsstList.add(new FnaAssetBusiandpersdets(strPKId,fnaselfspsDets,"PERSONAL",strArrPerAcctHolder[id],strArrPerTypeOfAsset[id],strArrPerNameOfAsset[id],dblPerPurInvValue,dblPerCurrValue,dblPerOsValue,dblPerEstApprValue,strArrPerYrs2keep[id],dblPerChildEdnPrcnt,dblPerRetPlanPrcnt,strArrPerRemarks[id],strArrPerCrtdBy[id],crtdDate,strLoggedUser,new Date()));
...
//	if(strArrBuisAsstMode != null && strArrBuisAsstMode.length>0){
		...
		if(strArrBuisAsstMode != null && strArrBuisAsstMode.length>0){
			...
			for(int id=0;id<strArrBuisAsstMode.length;id++){
				...
				double dblBuisEstApprValue = Double.parseDouble(FipaUtils.nullOrBlank(strArrBuisEstApprValue[id]) ? "0" :strArrBuisEstApprValue[id]);
				double dblBuisPurInvValue = Double.parseDouble(FipaUtils.nullOrBlank(strArrBuisPurInvValue[id]) ? "0" :strArrBuisPurInvValue[id]);
				double dblBuisCurrValue = Double.parseDouble(FipaUtils.nullOrBlank(strArrBuisCurrValue[id]) ? "0" :strArrBuisCurrValue[id]);
				double dblBuisOsValue = Double.parseDouble(FipaUtils.nullOrBlank(strArrBuisOsValue[id]) ? "0" :strArrBuisOsValue[id]);
				    
				double  dblBuisChildEdnPrcnt = Double.parseDouble(FipaUtils.nullOrBlank(strArrBuisChildEdnPrcnt[id]) ? "0" :strArrBuisChildEdnPrcnt[id]);
				double  dblBuisRetPlanPrcnt = Double.parseDouble(FipaUtils.nullOrBlank(strArrBuisRetPlanPrcnt[id]) ? "0" :strArrBuisRetPlanPrcnt[id]);
				
				Date crtdDate = FipaUtils.nullOrBlank(strArrBuisCrtdDate[id]) ? new Date() :FipaDateUtils.convertStringToDate(strArrBuisCrtdDate[id]) ;
				...
				if(strArrBuisAsstMode[id].equalsIgnoreCase(FipaConstant.INSERT_MODE)&& FipaUtils.nullOrBlank(strPKId)){
					insAsstList.add(new FnaAssetBusiandpersdets(null,null,"BUSINESS",strArrBuisAcctHolder[id],strArrBuisTypeOfAsset[id],strArrBuisNameOfAsset[id],dblBuisPurInvValue,dblBuisCurrValue,dblBuisOsValue,dblBuisEstApprValue,strArrBuisYrs2keep[id],dblBuisChildEdnPrcnt,dblBuisRetPlanPrcnt,strArrBuisRemarks[id],strLoggedUser,new Date(),null,null));
	 
				}
				
				if(strArrBuisAsstMode[id].equalsIgnoreCase(FipaConstant.UPDATE_MODE) || 
						(strArrBuisAsstMode[id].equalsIgnoreCase(FipaConstant.INSERT_MODE) && !FipaUtils.nullOrBlank(strPKId))){
					if(strPKId.startsWith("AS")){
						updAsstListAS.add(new FnaAssetBusiandpersdets(strPKId,fnaselfspsDets,"BUSINESS",strArrBuisAcctHolder[id],strArrBuisTypeOfAsset[id],strArrBuisNameOfAsset[id],dblBuisPurInvValue,dblBuisCurrValue,dblBuisOsValue,dblBuisEstApprValue,strArrBuisYrs2keep[id],dblBuisChildEdnPrcnt,dblBuisRetPlanPrcnt,strArrBuisRemarks[id],strLoggedUser,new Date(),strLoggedUser,new Date()));
						 
					}else{
						updAsstList.add(new FnaAssetBusiandpersdets(strPKId,fnaselfspsDets,"BUSINESS",strArrBuisAcctHolder[id],strArrBuisTypeOfAsset[id],strArrBuisNameOfAsset[id],dblBuisPurInvValue,dblBuisCurrValue,dblBuisOsValue,dblBuisEstApprValue,strArrBuisYrs2keep[id],dblBuisChildEdnPrcnt,dblBuisRetPlanPrcnt,strArrBuisRemarks[id],strArrBuisCrtdBy[id],crtdDate,strLoggedUser,new Date()));
				...
				if(strArrBuisAsstMode[id].equalsIgnoreCase(FipaConstant.DELETE_MODE)){
					delAsstList.add(new FnaAssetBusiandpersdets(strPKId,fnaselfspsDets,"BUSINESS",strArrBuisAcctHolder[id],strArrBuisTypeOfAsset[id],strArrBuisNameOfAsset[id],dblBuisPurInvValue,dblBuisCurrValue,dblBuisOsValue,dblBuisEstApprValue,strArrBuisYrs2keep[id],dblBuisChildEdnPrcnt,dblBuisRetPlanPrcnt,strArrBuisRemarks[id],strArrBuisCrtdBy[id],crtdDate,strLoggedUser,new Date()));
...
//		vectAsstDetails.add(updAsstListAS);
			...
			logger.debug("Error in getAssetDetails ->" , ex);
		}
	...
	public Vector getHlthInsDetails(HttpServletRequest request) {
		...
		List updHIList = new ArrayList();List updHIListAS = new ArrayList();
		...
		if(strArrHIMode != null && strArrHIMode.length>0){
			...
			for(int id=0;id<strArrHIMode.length;id++){
				...
				double dblHIAmt = Double.parseDouble(FipaUtils.nullOrBlank(strArrHIAmt[id]) ? "0" :strArrHIAmt[id]);
				double dblHIAnnPrm = Double.parseDouble(FipaUtils.nullOrBlank(strArrHIAnnPrm[id]) ? "0" :strArrHIAnnPrm[id]);
				
				Date crtdDate = FipaUtils.nullOrBlank(strArrHICrdtDate[id]) ? new Date() :FipaDateUtils.convertStringToDate(strArrHICrdtDate[id]) ;
				Date ExpiryDate = FipaUtils.nullOrBlank(strArrHIExp[id]) ?null :FipaDateUtils.convertStringToDate(strArrHIExp[id]) ;
				...
				if(strArrHIMode[id].equalsIgnoreCase(FipaConstant.INSERT_MODE) && FipaUtils.nullOrBlank(strPKId)){
					insHIList.add(new FnaHealthinsInfo(null,null,strArrHIPol[id],strArrHIIns[id],strArrHIBenf[id],dblHIAmt,ExpiryDate,dblHIAnnPrm,strArrHIRmks[id],strLoggedUser,new Date(),null,null));
	 
				}
				
				if(strArrHIMode[id].equalsIgnoreCase(FipaConstant.UPDATE_MODE)|| 
						(strArrHIMode[id].equalsIgnoreCase(FipaConstant.INSERT_MODE) && !FipaUtils.nullOrBlank(strPKId))){
					
					if(strPKId.startsWith("AS")){
						updHIListAS.add(new FnaHealthinsInfo(strArrHIId[id],fnaselfspsDets,strArrHIPol[id],strArrHIIns[id],strArrHIBenf[id],dblHIAmt,ExpiryDate,dblHIAnnPrm,strArrHIRmks[id],strLoggedUser,new Date(),strLoggedUser,new Date()));
						 
					}else{
						updHIList.add(new FnaHealthinsInfo(strArrHIId[id],fnaselfspsDets,strArrHIPol[id],strArrHIIns[id],strArrHIBenf[id],dblHIAmt,ExpiryDate,dblHIAnnPrm,strArrHIRmks[id],strArrHICrtdBy[id],crtdDate,strLoggedUser,new Date()));
				...
				if(strArrHIMode[id].equalsIgnoreCase(FipaConstant.DELETE_MODE)){
					delHIList.add(new FnaHealthinsInfo(strArrHIId[id],fnaselfspsDets,strArrHIPol[id],strArrHIIns[id],strArrHIBenf[id],dblHIAmt,ExpiryDate,dblHIAnnPrm,strArrHIRmks[id],strArrHICrtdBy[id],crtdDate,strLoggedUser,new Date()));
		...
		vectHIDetails.add(delHIList);vectHIDetails.add(updHIListAS);
		}catch(Exception ex){
			logger.debug("Error in getHlthInsDetails ->" , ex);
		}
	...
	//	
	// 
	//	String[] strArrContgDepMode   	  = FipaUtils.getParamArrValue(request, "txtFldContgDepMode");
	//	String[] strArrContgDepId    	  = FipaUtils.getParamArrValue(request, "txFldContgDepId");
	//	String[] strArrContgDepName  	  = FipaUtils.getParamArrValue(request, "txtFldContgDepName");
	//	String[] strArrContgDepAge   	  = FipaUtils.getParamArrValue(request, "txtFldContgDepAge");
	//	String[] strArrContgDepYrs   	  = FipaUtils.getParamArrValue(request, "txtFldContgDepYrs");
	//	String[] strArrContgDepLvNds 	  = FipaUtils.getParamArrValue(request, "txtFldContgDepLivingNeeds");
	//	String[] strArrContgDepRel        = FipaUtils.getParamArrValue(request, "selContgDepRelationship");
	//	String[] strArrContgDepYrster     = FipaUtils.getParamArrValue(request, "txtFldContgDepYrsteritary");
	//	String[] strArrContgDepEdnyrs	  = FipaUtils.getParamArrValue(request, "txtFldContgDepEdnyrs");		
	//	String[] strArrContgDepEdnYAnlCst = FipaUtils.getParamArrValue(request, "txtFldContgDepEdnyrsAnnlCost");
	//	String[] strArrContgDepEdnFund    = FipaUtils.getParamArrValue(request, "txtFldContgDepEdnFund"); 
	//	String[] strArrContgDepRb         = FipaUtils.getParamArrValue(request, "txtFldContgDepRb");
	//	String[] strArrContgDepWrd        = FipaUtils.getParamArrValue(request, "txtFldContgDepWrd");
	//	String[] strArrContgDepHospType   = FipaUtils.getParamArrValue(request, "txtFldContgDepHospType"); 
	//	String[] strArrContgDepCrtdBy     = FipaUtils.getParamArrValue(request, "txtFldContgDepCrtdBy");
	//	String[] strArrContgDepCrtdDate   = FipaUtils.getParamArrValue(request, "txtFldContgDepCrtdDate");
	// 
	//	String strFNAId = FipaUtils.getParamValue(request, "fnaId");
	//	String strLoggedUser = (String) sessMap.get(FipaConstant.LOGGED_USER_ID);
	//	
	...
	//			
	//			short  shtContgDepYrs = Short.parseShort(FipaUtils.nullOrBlank(strArrContgDepYrs[id]) ? "0" :strArrContgDepYrs[id]);
	//			short  shtContgDepYrster = Short.parseShort(FipaUtils.nullOrBlank(strArrContgDepYrster[id]) ? "0" :strArrContgDepYrster[id]);
	//			short  shtContgDepEdnyrs = Short.parseShort(FipaUtils.nullOrBlank(strArrContgDepEdnyrs[id]) ? "0" :strArrContgDepEdnyrs[id]);
	//			
	//			double dblContgDepEdnACst = Double.parseDouble(FipaUtils.nullOrBlank(strArrContgDepEdnYAnlCst[id]) ? "0" :strArrContgDepEdnYAnlCst[id]);
	//			double dblContgDepEdnFund = Double.parseDouble(FipaUtils.nullOrBlank(strArrContgDepEdnFund[id]) ? "0" :strArrContgDepEdnFund[id]);
	//			
	//			Date crtdDate = FipaUtils.nullOrBlank(strArrContgDepCrtdDate[id]) ? new Date() :FipaDateUtils.convertStringToDate(strArrContgDepCrtdDate[id]) ;
	//			
	//		
	//			
	//			FnaSelfspouseDets fnaselfspsDets = new FnaSelfspouseDets();				
	...
	//				insContgDepList.add(new FnaContinDepnDets(null,null,strArrContgDepName[id],strArrContgDepAge[id],shtContgDepYrs,strArrContgDepLvNds[id],strArrContgDepRel[id],shtContgDepYrster,shtContgDepEdnyrs,dblContgDepEdnACst,dblContgDepEdnFund,strArrContgDepRb[id],strArrContgDepWrd[id],strArrContgDepHospType[id],strLoggedUser,new Date(),null,null,null));
	//		 
	//			}
	//			
	//			if(strArrContgDepMode[id].equalsIgnoreCase(FipaConstant.UPDATE_MODE)){
	//				updContgDepList.add(new FnaContinDepnDets(strArrContgDepId[id],fnaselfspsDets,strArrContgDepName[id],strArrContgDepAge[id],shtContgDepYrs,strArrContgDepLvNds[id],strArrContgDepRel[id],shtContgDepYrster,shtContgDepEdnyrs,dblContgDepEdnACst,dblContgDepEdnFund,strArrContgDepRb[id],strArrContgDepWrd[id],strArrContgDepHospType[id],strArrContgDepCrtdBy[id],crtdDate,strLoggedUser,new Date(),null));
	//		 
	//			}
	//			
	//			if(strArrContgDepMode[id].equalsIgnoreCase(FipaConstant.DELETE_MODE)){
	//				
	//				delContgDepList.add(new FnaContinDepnDets(strArrContgDepId[id],fnaselfspsDets,strArrContgDepName[id],strArrContgDepAge[id],shtContgDepYrs,strArrContgDepLvNds[id],strArrContgDepRel[id],shtContgDepYrster,shtContgDepEdnyrs,dblContgDepEdnACst,dblContgDepEdnFund,strArrContgDepRb[id],strArrContgDepWrd[id],strArrContgDepHospType[id],strArrContgDepCrtdBy[id],crtdDate,strLoggedUser,new Date(),null));
	//		 
	//			}
	//		}
	//	}
	...
	//	sessMap = (HashMap<String, String>)session.getAttribute(FipaConstant.LOGGED_USER_INFO);
	//	
	//	String[] strArrFdFlwMode  = FipaUtils.getParamArrValue(request, "txtFldFdFlwMode");
	//	String[] strArrFlowId     = FipaUtils.getParamArrValue(request, "txtFldFlowId");
	//	String[] strArrFlowCtg    = FipaUtils.getParamArrValue(request, "txtFldFlowCateg");
	//	String[] strArrFlowSrc    = FipaUtils.getParamArrValue(request, "txtFldFlowSource");
	//	String[] strArrFlowWhen   = FipaUtils.getParamArrValue(request, "txtFldFlowWhen");
	//	String[] strArrFlowAmt    = FipaUtils.getParamArrValue(request, "txtFldFlowAmt");
	//	String[] strArrFlowRmrks  = FipaUtils.getParamArrValue(request, "txtFldFlowRemarks");
	//	String[] strArrFlowCrtdBy = FipaUtils.getParamArrValue(request, "txtFldFlowCrtdBy");
	//	String[] strArrFlowCrtdDate = FipaUtils.getParamArrValue(request, "txtFldFlowCrtdDate");		
	//
	//	
	//	
	//	String strFNAId = FipaUtils.getParamValue(request, "fnaId");
	...
	//			
	//			short shtFlowWhen = Short.parseShort(FipaUtils.nullOrBlank(strArrFlowWhen[id]) ? "0" :strArrFlowWhen[id]);
	//			double dblFlowAmt = Double.parseDouble(FipaUtils.nullOrBlank(strArrFlowAmt[id]) ? "0" :strArrFlowAmt[id]);
	//				
	//			Date crtdDate = FipaUtils.nullOrBlank(strArrFlowCrtdDate[id]) ? new Date() :FipaDateUtils.convertStringToDate(strArrFlowCrtdDate[id]) ;
	//			
	//			FnaSelfspouseDets fnaselfspsDets = new FnaSelfspouseDets();				
	//			fnaselfspsDets.setFnaId(strFNAId);
	//			
	//			if(strArrFdFlwMode[id].equalsIgnoreCase(FipaConstant.INSERT_MODE)){
	//				insFdFlwList.add(new FnaFlowDets(null,null,strArrFlowCtg[id],strArrFlowSrc[id],shtFlowWhen,dblFlowAmt,strArrFlowRmrks[id],strLoggedUser,new Date(),null,null));
	//		 
	//			}
	//			
	//			if(strArrFdFlwMode[id].equalsIgnoreCase(FipaConstant.UPDATE_MODE)){
	//				updFdFlwList.add(new FnaFlowDets(strArrFlowId[id],fnaselfspsDets,strArrFlowCtg[id],strArrFlowSrc[id],shtFlowWhen,dblFlowAmt,strArrFlowRmrks[id],strArrFlowCrtdBy[id],crtdDate,strLoggedUser,new Date()));
	//		 
	//			}
	//			
	//			if(strArrFdFlwMode[id].equalsIgnoreCase(FipaConstant.DELETE_MODE)){
	//				delFdFlwList.add(new FnaFlowDets(strArrFlowId[id],fnaselfspsDets,strArrFlowCtg[id],strArrFlowSrc[id],shtFlowWhen,dblFlowAmt,strArrFlowRmrks[id],strArrFlowCrtdBy[id],crtdDate,strLoggedUser,new Date()));
	//		 
	//			}
	//		}
	//	}
	...
	//	 
	//
	//	String[] strArrInsrcMode 		= FipaUtils.getParamArrValue(request, "txtFldInsrcMode");
	//	String[] strArrLipId 			= FipaUtils.getParamArrValue(request, "lipId");
	//	String[] strArrLipOwner 		= FipaUtils.getParamArrValue(request, "txtFldLipOwner");
	//	String[] strArrLipAssured 		= FipaUtils.getParamArrValue(request, "txtFldLipAssured");
	//	String[] strArrLipCompany 		= FipaUtils.getParamArrValue(request, "txtFldLipCompany");
	//	String[] strArrLipPolNo 		= FipaUtils.getParamArrValue(request, "txtFldLipPolicyNo");
	//	String[] strArrLipPolType 		= FipaUtils.getParamArrValue(request, "txtFldLipPolicyType");
	//	String[] strArrLipDthchBenf 	= FipaUtils.getParamArrValue(request, "txtFldLipDthchBenf");
	//	String[] strArrLipMiCovrge 		= FipaUtils.getParamArrValue(request, "txtFldLipMiCoverage");		
	//	String[] strArrLipCashVal 		= FipaUtils.getParamArrValue(request, "txtFldLipCashValue");
	//	String[] strArrLipMatVal 		= FipaUtils.getParamArrValue(request, "txtFldLipMaturityValue"); 
	//	String[] strArrLipOsVal		 	= FipaUtils.getParamArrValue(request, "txtFldLipOsValue");
	//	String[] strArrLipMatDate 		= FipaUtils.getParamArrValue(request, "lipMaturityDate");
	//	String[] strArrLipEdnVal 		= FipaUtils.getParamArrValue(request, "txtFldLipEdnValue"); 
	//	String[] strArrLipRpValue 		= FipaUtils.getParamArrValue(request, "txtFldLipRpValue");
	//	String[] strArrLipCpfValue 		= FipaUtils.getParamArrValue(request, "txtFldLipCpfValue");
	//	String[] strArrLipAnnlPrem 		= FipaUtils.getParamArrValue(request, "txtFldLipAnnlPrem");
	//	String[] strArrLipTpdPay1		= FipaUtils.getParamArrValue(request, "txtFldLipTpdPay1");
	//	String[] strArrLipYrto1 		= FipaUtils.getParamArrValue(request, "txtFldLipYrto1");
	//	String[] strArrLipYr1 			= FipaUtils.getParamArrValue(request, "txtFldLipYr1");
	//	String[] strArrLipTpdPay2 		= FipaUtils.getParamArrValue(request, "txtFldLipTpdPay2");		
	//	String[] strArrLipYrto2 		= FipaUtils.getParamArrValue(request, "txtFldLipYrto2");
	//	String[] strArrLipYr2 			= FipaUtils.getParamArrValue(request, "txtFldLipYr2"); 
	//	String[] strArrLipTpdPay3 		= FipaUtils.getParamArrValue(request, "txtFldLipTpdPay3");
	//	String[] strArrLipYrto3 		= FipaUtils.getParamArrValue(request, "txtFldLipYrto3");
	//	String[] strArrLipYr3 			= FipaUtils.getParamArrValue(request, "txtFldLipYr3"); 
	//	String[] strArrLipComputedtpd 	= FipaUtils.getParamArrValue(request, "txtFldLipComputedtpd");
	//	String[] strArrLipRmrks 		= FipaUtils.getParamArrValue(request, "txtFldLipRmrks");
	////	String[] strArrLipOwnerType 	= FipaUtils.getParamArrValue(request, "selOwnerType");
	//	String[] strArrLipCrtdBy 		= FipaUtils.getParamArrValue(request, "lipCreatedBy");
	//	String[] strArrLipCrtdDate 		= FipaUtils.getParamArrValue(request, "lipCreatedDate");
	//    String[] strArrLipModBy 		= FipaUtils.getParamArrValue(request, "txtFldLipModBy");
	//	String[] strArrLipModDate 		= FipaUtils.getParamArrValue(request, "txtFldLipModDate");
	//	 
	//	String strFNAId = FipaUtils.getParamValue(request, "fnaId");
	//	String strLoggedUser = (String) sessMap.get(FipaConstant.LOGGED_USER_ID);
	//	
	...
	//			 
	//			double dblLipDthchBenf = Double.parseDouble(FipaUtils.nullOrBlank(strArrLipDthchBenf[id]) ? "0" :strArrLipDthchBenf[id]);
	//			double dblLipMiCoverage = Double.parseDouble(FipaUtils.nullOrBlank(strArrLipMiCovrge[id]) ? "0" :strArrLipMiCovrge[id]);
	//			double dblLipCashValue= Double.parseDouble(FipaUtils.nullOrBlank(strArrLipCashVal[id]) ? "0" :strArrLipCashVal[id]);
	//		    double dblLipMaturityValue= Double.parseDouble(FipaUtils.nullOrBlank(strArrLipMatVal[id]) ? "0" :strArrLipMatVal[id]);
	//		    double dblLipOsValue= Double.parseDouble(FipaUtils.nullOrBlank(strArrLipOsVal[id]) ? "0" :strArrLipOsVal[id]);
	//		    double dblLipEdnValue= Double.parseDouble(FipaUtils.nullOrBlank(strArrLipEdnVal[id]) ? "0" :strArrLipEdnVal[id]);
	//		    double dblLipRpValue= Double.parseDouble(FipaUtils.nullOrBlank(strArrLipRpValue[id]) ? "0" :strArrLipRpValue[id]);
	//		    double dblLipCpfValue= Double.parseDouble(FipaUtils.nullOrBlank(strArrLipCpfValue[id]) ? "0" :strArrLipCpfValue[id]);
	//		    double dblLipAnnlPrem= Double.parseDouble(FipaUtils.nullOrBlank(strArrLipAnnlPrem[id]) ? "0" :strArrLipAnnlPrem[id]);
	//		    double dblLipTpdPay1= Double.parseDouble(FipaUtils.nullOrBlank(strArrLipTpdPay1[id]) ? "0" :strArrLipTpdPay1[id]);
	//		    double dblLipYrto1= Double.parseDouble(FipaUtils.nullOrBlank(strArrLipYrto1[id]) ? "0" :strArrLipYrto1[id]);
	//		    double dblLipYr1= Double.parseDouble(FipaUtils.nullOrBlank(strArrLipYr1[id]) ? "0" :strArrLipYr1[id]);
	//		    double dblLipTpdPay2= Double.parseDouble(FipaUtils.nullOrBlank(strArrLipTpdPay2[id]) ? "0" :strArrLipTpdPay2[id]);
	//		    double dblLipYrto2= Double.parseDouble(FipaUtils.nullOrBlank(strArrLipYrto2[id]) ? "0" :strArrLipYrto2[id]);
	//		    double dblLipYr2= Double.parseDouble(FipaUtils.nullOrBlank(strArrLipYr2[id]) ? "0" :strArrLipYr2[id]);
	//		    double dblLipTpdPay3= Double.parseDouble(FipaUtils.nullOrBlank(strArrLipTpdPay3[id]) ? "0" :strArrLipTpdPay3[id]);
	//		    double dblLipYrto3= Double.parseDouble(FipaUtils.nullOrBlank(strArrLipYrto3[id]) ? "0" :strArrLipYrto3[id]);
	//		    double dblLipYr3= Double.parseDouble(FipaUtils.nullOrBlank(strArrLipYr3[id]) ? "0" :strArrLipYr3[id]);
	//		    double dblLipComputedtpd= Double.parseDouble(FipaUtils.nullOrBlank(strArrLipComputedtpd[id]) ? "0" :strArrLipComputedtpd[id]);
	//			
	// 	
	//			Date crtdDate = FipaUtils.nullOrBlank(strArrLipCrtdDate[id]) ? new Date() :FipaDateUtils.convertStringToDate(strArrLipCrtdDate[id]);
	//			Date dteLipMaturity = FipaUtils.nullOrBlank(strArrLipMatDate[id]) ? new Date() :FipaDateUtils.convertStringToDate(strArrLipMatDate[id]) ;
	//			
	//			FnaSelfspouseDets fnaselfspsDets = new FnaSelfspouseDets();				
	//			fnaselfspsDets.setFnaId(strFNAId);
	//			
	//			if(strArrInsrcMode[id].equalsIgnoreCase(FipaConstant.INSERT_MODE)){
	//				insInsList.add(new FnaLifeinsplansDets(null,null,strArrLipOwner[id],strArrLipAssured[id],strArrLipCompany[id],strArrLipPolNo[id],strArrLipPolType[id],dblLipDthchBenf,dblLipMiCoverage,dblLipCashValue,dblLipMaturityValue,dblLipOsValue,dteLipMaturity,dblLipEdnValue,dblLipRpValue,dblLipCpfValue,dblLipAnnlPrem,dblLipTpdPay1,dblLipYrto1,dblLipYr1,dblLipTpdPay2,dblLipYrto2,dblLipYr2,dblLipTpdPay3,dblLipYrto3,dblLipYr3,dblLipComputedtpd,strArrLipRmrks[id],strLoggedUser,new Date(),null,null,""));
	//				
	//			}
	//			
	//			if(strArrInsrcMode[id].equalsIgnoreCase(FipaConstant.UPDATE_MODE)){
	//				updInsList.add(new FnaLifeinsplansDets(strArrLipId[id],fnaselfspsDets,strArrLipOwner[id],strArrLipAssured[id],strArrLipCompany[id],strArrLipPolNo[id],strArrLipPolType[id],dblLipDthchBenf,dblLipMiCoverage,dblLipCashValue,dblLipMaturityValue,dblLipOsValue,dteLipMaturity,dblLipEdnValue,dblLipRpValue,dblLipCpfValue,dblLipAnnlPrem,dblLipTpdPay1,dblLipYrto1,dblLipYr1,dblLipTpdPay2,dblLipYrto2,dblLipYr2,dblLipTpdPay3,dblLipYrto3,dblLipYr3,dblLipComputedtpd,strArrLipRmrks[id],strLoggedUser,new Date(),null,null,""));
	//			 
	//			}
	//			
	//			if(strArrInsrcMode[id].equalsIgnoreCase(FipaConstant.DELETE_MODE)){
	//				delInsList.add(new FnaLifeinsplansDets(strArrLipId[id],fnaselfspsDets,strArrLipOwner[id],strArrLipAssured[id],strArrLipCompany[id],strArrLipPolNo[id],strArrLipPolType[id],dblLipDthchBenf,dblLipMiCoverage,dblLipCashValue,dblLipMaturityValue,dblLipOsValue,dteLipMaturity,dblLipEdnValue,dblLipRpValue,dblLipCpfValue,dblLipAnnlPrem,dblLipTpdPay1,dblLipYrto1,dblLipYr1,dblLipTpdPay2,dblLipYrto2,dblLipYr2,dblLipTpdPay3,dblLipYrto3,dblLipYr3,dblLipComputedtpd,strArrLipRmrks[id],strLoggedUser,new Date(),null,null,""));
	//			 
	//			}
	//		}
	//	}
	...
	//	
	//  
	//	String[] strArrIpInvstMode = FipaUtils.getParamArrValue(request, "txtFldIpInvstMode");
	//	String[] strArrOwnInvId = FipaUtils.getParamArrValue(request, "txtFldOwnInvId");
	//	String[] strArrInvstOwnerType = FipaUtils.getParamArrValue(request, "txtFldInvstOwnerType");
	//	String[] strArrInvstType = FipaUtils.getParamArrValue(request, "txtFldInvstType");
	//	String[] strArrInvstInstName = FipaUtils.getParamArrValue(request, "txtFldInvstInstName");
	//	String[] strArrInvstDesc = FipaUtils.getParamArrValue(request, "txtFldInvstDesc");
	//	String[] strArrInvstAmt = FipaUtils.getParamArrValue(request, "txtFldInvstAmt");
	//	String[] strArrInvstPayMtd = FipaUtils.getParamArrValue(request, "txtFldInvstPayMtd");
	//	String[] strArrInvstCurrBid = FipaUtils.getParamArrValue(request, "txtFldInvstCurrBid");		
	//	String[] strArrInvstDate = FipaUtils.getParamArrValue(request, "txtFldInvstDate");
	//	String[] strArrInvstUnitsAlloc = FipaUtils.getParamArrValue(request, "txtFldInvstUnitsAlloc"); 
	//	String[] strArrInvstCurrnav = FipaUtils.getParamArrValue(request, "txtFldInvstCurrnav");
	//	String[] strArrInvstAnnlTopup = FipaUtils.getParamArrValue(request, "txtFldInvstAnnlTopup");
	//	String[] strArrInvstCpfyrstoInvst = FipaUtils.getParamArrValue(request, "txtFldInvstCpfyrstoInvst"); 
	//	String[] strArrInvstFvtoCpf = FipaUtils.getParamArrValue(request, "txtFldInvstFvtoCpf");
	//	String[] strArrInvstRptoInv = FipaUtils.getParamArrValue(request, "txtFldInvstRptoInv");
	//	String[] strArrInvstRetCashPrcnt = FipaUtils.getParamArrValue(request, "txtFldInvstRetCashPrcnt");
	//	String[] strArrInvstFvofCashsrs = FipaUtils.getParamArrValue(request, "txtFldInvstFvofCashsrs");
	//	String[] strArrInvstEdntoInv = FipaUtils.getParamArrValue(request, "txtFldInvstEdntoInv");
	//	String[] strArrInvstEdnCashPrcnt = FipaUtils.getParamArrValue(request, "txtFldInvstEdnCashPrcnt");
	//	String[] strArrInvstChildEdnInv = FipaUtils.getParamArrValue(request, "txtFldInvstChildEdnInv");		
	//	String[] strArrInvstChildName = FipaUtils.getParamArrValue(request, "txtFldInvstChildName");
	//	String[] strArrInvstRmrks = FipaUtils.getParamArrValue(request, "txtFldInvstRmrks"); 
	//	String[] strArrInvstOwner = FipaUtils.getParamArrValue(request, "txtFldInvstOwner");
	//	String[] strArrInvstCrtdBy = FipaUtils.getParamArrValue(request, "txtFldInvstCrtdBy");
	//	String[] strArrInvstCrtdDate = FipaUtils.getParamArrValue(request, "txtFldInvstCrtdDate"); 
	//	String[] strArrInvstModBy = FipaUtils.getParamArrValue(request, "txtFldInvstModBy");
	//	String[] strArrInvstModDate = FipaUtils.getParamArrValue(request, "txtFldInvstModDate");
	//	 
	//	
	//	String strFNAId = FipaUtils.getParamValue(request, "fnaId");
	//	String strLoggedUser = (String) sessMap.get(FipaConstant.LOGGED_USER_ID); 
	...
	//			 
	//			short shtInvstCpfyrstoInvst = Short.parseShort(FipaUtils.nullOrBlank(strArrInvstCpfyrstoInvst[id]) ? "0" :strArrInvstCpfyrstoInvst[id]);
	//			short shtInvstRetCashPrcnt = Short.parseShort(FipaUtils.nullOrBlank(strArrInvstRetCashPrcnt[id]) ? "0" :strArrInvstRetCashPrcnt[id]);
	//			short shtInvstEdnCashPrcnt = Short.parseShort(FipaUtils.nullOrBlank(strArrInvstEdnCashPrcnt[id]) ? "0" :strArrInvstEdnCashPrcnt[id]);
	//			
	//			double dblInvstAmt= Double.parseDouble(FipaUtils.nullOrBlank(strArrInvstAmt[id]) ? "0" :strArrInvstAmt[id]);
	//			double dblInvstCurrBid = Double.parseDouble(FipaUtils.nullOrBlank(strArrInvstCurrBid[id]) ? "0" :strArrInvstCurrBid[id]);
	//			double dblInvstUnitsAlloc = Double.parseDouble(FipaUtils.nullOrBlank(strArrInvstUnitsAlloc[id]) ? "0" :strArrInvstUnitsAlloc[id]);
	//			double dblInvstCurrnav= Double.parseDouble(FipaUtils.nullOrBlank(strArrInvstCurrnav[id]) ? "0" :strArrInvstCurrnav[id]);
	//		    double dblInvstAnnlTopup= Double.parseDouble(FipaUtils.nullOrBlank(strArrInvstAnnlTopup[id]) ? "0" :strArrInvstAnnlTopup[id]);
	//		    double dblInvstFvtoCpf= Double.parseDouble(FipaUtils.nullOrBlank(strArrInvstFvtoCpf[id]) ? "0" :strArrInvstFvtoCpf[id]);
	//		    double dblInvstRptoInv= Double.parseDouble(FipaUtils.nullOrBlank(strArrInvstRptoInv[id]) ? "0" :strArrInvstRptoInv[id]);
	//		    double dblInvstFvofCashsrs = Double.parseDouble(FipaUtils.nullOrBlank(strArrInvstFvofCashsrs[id]) ? "0" :strArrInvstFvofCashsrs[id]);
	//		    double dblInvstEdntoInv = Double.parseDouble(FipaUtils.nullOrBlank(strArrInvstEdntoInv[id]) ? "0" :strArrInvstEdntoInv[id]);
	//		    double dblInvstChildEdnInv= Double.parseDouble(FipaUtils.nullOrBlank(strArrInvstChildEdnInv[id]) ? "0" :strArrInvstChildEdnInv[id]);
	//		   
	//			Date crtdDate = FipaUtils.nullOrBlank(strArrInvstCrtdDate[id]) ? new Date() :FipaDateUtils.convertStringToDate(strArrInvstCrtdDate[id]);
	//			Date dteInvstDate = FipaUtils.nullOrBlank(strArrInvstDate[id]) ? new Date() :FipaDateUtils.convertStringToDate(strArrInvstDate[id]) ;
	//			
	//			FnaSelfspouseDets fnaselfspsDets = new FnaSelfspouseDets();				
	//			fnaselfspsDets.setFnaId(strFNAId);
	//			
	//			if(strArrIpInvstMode[id].equalsIgnoreCase(FipaConstant.INSERT_MODE)){
	//				insIpInvstList.add(new FnaInputinvsetDets(null,null,strArrInvstOwnerType[id],strArrInvstType[id],strArrInvstInstName[id],strArrInvstDesc[id],dblInvstAmt,strArrInvstPayMtd[id],dblInvstCurrBid,dteInvstDate,dblInvstUnitsAlloc,dblInvstCurrnav,dblInvstAnnlTopup,shtInvstCpfyrstoInvst,dblInvstFvtoCpf,dblInvstRptoInv,shtInvstRetCashPrcnt,dblInvstFvofCashsrs,dblInvstEdntoInv,shtInvstEdnCashPrcnt,dblInvstChildEdnInv,strArrInvstChildName[id],strArrInvstRmrks[id],strArrInvstOwner[id],strLoggedUser,new Date(),null,null));
	// 
	//			}
	//			
	//			if(strArrIpInvstMode[id].equalsIgnoreCase(FipaConstant.UPDATE_MODE)){
	//				updIpInvstList.add(new FnaInputinvsetDets(strArrOwnInvId[id],fnaselfspsDets,strArrInvstOwnerType[id],strArrInvstType[id],strArrInvstInstName[id],strArrInvstDesc[id],dblInvstAmt,strArrInvstPayMtd[id],dblInvstCurrBid,dteInvstDate,dblInvstUnitsAlloc,dblInvstCurrnav,dblInvstAnnlTopup,shtInvstCpfyrstoInvst,dblInvstFvtoCpf,dblInvstRptoInv,shtInvstRetCashPrcnt,dblInvstFvofCashsrs,dblInvstEdntoInv,shtInvstEdnCashPrcnt,dblInvstChildEdnInv,strArrInvstChildName[id],strArrInvstRmrks[id],strArrInvstOwner[id],strArrInvstCrtdBy[id],crtdDate,strLoggedUser,new Date()));
	//				
	// 
	//			}
	//			
	//			if(strArrIpInvstMode[id].equalsIgnoreCase(FipaConstant.DELETE_MODE)){
	//				delIpInvstList.add(new FnaInputinvsetDets(strArrOwnInvId[id],fnaselfspsDets,strArrInvstOwnerType[id],strArrInvstType[id],strArrInvstInstName[id],strArrInvstDesc[id],dblInvstAmt,strArrInvstPayMtd[id],dblInvstCurrBid,dteInvstDate,dblInvstUnitsAlloc,dblInvstCurrnav,dblInvstAnnlTopup,shtInvstCpfyrstoInvst,dblInvstFvtoCpf,dblInvstRptoInv,shtInvstRetCashPrcnt,dblInvstFvofCashsrs,dblInvstEdntoInv,shtInvstEdnCashPrcnt,dblInvstChildEdnInv,strArrInvstChildName[id],strArrInvstRmrks[id],strArrInvstOwner[id],strArrInvstCrtdBy[id],crtdDate,strLoggedUser,new Date()));
	// 
	//			}
	//		}
	//	}
	...
	//	
	//  
	//	 String[] strArrRcomMode 		= FipaUtils.getParamArrValue(request, "txtFldRcomMode");
	//	 String[] strArrRecomId 		= FipaUtils.getParamArrValue(request, "txtFldRecomId");
	//	 String[] strArrRecomProdType 	= FipaUtils.getParamArrValue(request, "txtFldRecomProdType");
	//	 String[] strArrRecomProdName	= FipaUtils.getParamArrValue(request, "txtFldRecomProdName");		
	//	 String[] strArrRecomSumAssr 	= FipaUtils.getParamArrValue(request, "txtFldRecomSumAssr");
	//	 String[] strArrRecomPayMode 	= FipaUtils.getParamArrValue(request, "txtFldRecomPayMode"); 
	//	 String[] strArrRecomPlanTerm 	= FipaUtils.getParamArrValue(request, "txtFldRecomPlanTerm");
	//	 String[] strArrRecomPayTerm 	= FipaUtils.getParamArrValue(request, "txtFldRecomPayTerm");
	//	 String[] strArrRecomPremium 	= FipaUtils.getParamArrValue(request, "txtFldRecomPremium"); 
	//	 String[] strArrRecomCrtdBy 	= FipaUtils.getParamArrValue(request, "txtFldRecomCrtdBy");
	//	 String[] strArrRecomCrtdDate 	= FipaUtils.getParamArrValue(request, "txtFldRecomCrtdDate"); 
	//	 String[] strArrRecomModBy 		= FipaUtils.getParamArrValue(request, "txtFldRecomModBy");
	//	 String[] strArrRecomModDate 	= FipaUtils.getParamArrValue(request, "txtFldRecomModDate");
	//	 
	//	
	//	String strFNAId = FipaUtils.getParamValue(request, "fnaId");
	//	String strLoggedUser = (String) sessMap.get(FipaConstant.LOGGED_USER_ID);
	...
	//			 
	//			double dblRcmSumAssr= Double.parseDouble(FipaUtils.nullOrBlank(strArrRecomSumAssr[id]) ? "0" :strArrRecomSumAssr[id]);
	//			double dblRcmPlanTerm = Double.parseDouble(FipaUtils.nullOrBlank(strArrRecomPlanTerm[id]) ? "0" :strArrRecomPlanTerm[id]);
	//			double dblRcmPremium = Double.parseDouble(FipaUtils.nullOrBlank(strArrRecomPremium[id]) ? "0" :strArrRecomPremium[id]);
	//			 
	//			short shtRcmPayTerm = Short.parseShort(FipaUtils.nullOrBlank(strArrRecomPayTerm[id]) ? "0" :strArrRecomPayTerm[id]);
	//			 
	//			
	//			Date crtdDate = FipaUtils.nullOrBlank(strArrRecomCrtdDate[id]) ? new Date() :FipaDateUtils.convertStringToDate(strArrRecomCrtdDate[id]);
	//			 
	//			FnaSelfspouseDets fnaselfspsDets = new FnaSelfspouseDets();				
	//			fnaselfspsDets.setFnaId(strFNAId);
	//			
	//			if(strArrRcomMode[id].equalsIgnoreCase(FipaConstant.INSERT_MODE)){
	//				insRcmPrdList.add(new FnaRecomPrdtplanDet(null,null,"","","ADVREC","","",strArrRecomProdType[id],strArrRecomProdName[id],dblRcmSumAssr,strArrRecomPayMode[id],dblRcmPlanTerm,shtRcmPayTerm,dblRcmPremium,strLoggedUser,new Date(),null,null));
	//				 
	//			}
	//			
	//			if(strArrRcomMode[id].equalsIgnoreCase(FipaConstant.UPDATE_MODE)){
	//				updRcmPrdList.add(new FnaRecomPrdtplanDet(strArrRecomId[id],fnaselfspsDets,"","","ADVREC","","",strArrRecomProdType[id],strArrRecomProdName[id],dblRcmSumAssr,strArrRecomPayMode[id],dblRcmPlanTerm,shtRcmPayTerm,dblRcmPremium,strArrRecomCrtdBy[id],crtdDate,strLoggedUser,new Date()));
	//				 
	//			}
	//			
	//			if(strArrRcomMode[id].equalsIgnoreCase(FipaConstant.DELETE_MODE)){
	//				delRcmPrdList.add(new FnaRecomPrdtplanDet(strArrRecomId[id],fnaselfspsDets,"","","ADVREC","","",strArrRecomProdType[id],strArrRecomProdName[id],dblRcmSumAssr,strArrRecomPayMode[id],dblRcmPlanTerm,shtRcmPayTerm,dblRcmPremium,strArrRecomCrtdBy[id],crtdDate,strLoggedUser,new Date()));
	//				 
	//			}
	//		}
	//	}
...
//		vectRcmPrdPlnDetails.add(delRcmPrdPlnList);
			...
			logger.debug("Error in getRcmPrdPlnDetails ->" , ex);
		}
...
//		vectRcmPrdFundDetails.add(delRcmPrdFundList);
			...
			logger.debug("Error in getRcmPrdFundDetails ->" , ex);
		}
...
//		vectSwtPlnDetails.add(delSwtPlnList);
			...
			logger.debug("Error in getSwtPlnDetails ->" , ex);
		}
...
//		vectSwtFundDetails.add(delSwtFundList);
			...
			logger.debug("Error in getSwtFundDetails ->" , ex);
		}
	...
	//	fnaselfspsDets.setFnaId(strFNAId);	
	//	
	//	String strCDBenfName = request.getParameter("amlBenfName") != null ? request.getParameter("amlBenfName") : null;
	//	fnaOthPerDets.setAmlBenfName(strCDBenfName);
	//	String strCDBenfNRIC = request.getParameter("amlBenfNric") != null ? request.getParameter("amlBenfNric") : null;
	//	fnaOthPerDets.setAmlBenfNric(strCDBenfNRIC); 
	//	String strCDBenfIncorpNo = request.getParameter("amlBenfIncno") != null ? request.getParameter("amlBenfIncno") : null;
	//	fnaOthPerDets.setAmlBenfIncno(strCDBenfIncorpNo); 
	//	String strCDBenfRegAddr = request.getParameter("amlBenfAddr1") != null ? request.getParameter("amlBenfAddr1") : null;
	//	fnaOthPerDets.setAmlBenfAddr1(strCDBenfRegAddr); 
	//	String strCDBenfWork = request.getParameter("amlBenfJob") != null ? request.getParameter("amlBenfJob") : null;
	//	fnaOthPerDets.setAmlBenfJob(strCDBenfWork); 
	//	String strCDBenfWrkCountry = request.getParameter("amlBenfJobconty") != null ? request.getParameter("amlBenfJobconty") : null;
	//	fnaOthPerDets.setAmlBenfJobconty(strCDBenfWrkCountry); 
	//	String strCDBenfRelation = request.getParameter("amlBenfRelat") != null ? request.getParameter("amlBenfRelat") : null;
	//	fnaOthPerDets.setAmlBenfRelat(strCDBenfRelation);  
	//	String strCDBenfRegAddr2 = request.getParameter("amlBenfAddr2") != null ? request.getParameter("amlBenfAddr2") : null;
	//	fnaOthPerDets.setAmlBenfAddr2(strCDBenfRegAddr2); 
	//	String strCDBenfPostal = request.getParameter("amlBenfPostal") != null ? request.getParameter("amlBenfPostal") : null;
	//	fnaOthPerDets.setAmlBenfPostal(strCDBenfPostal); 
	//	String strCDBenfCntry = request.getParameter("amlBenfConty") != null ? request.getParameter("amlBenfConty") : null;
	//	fnaOthPerDets.setAmlBenfConty(strCDBenfCntry);
	//	
	//	if(!(FipaUtils.nullOrBlank(strCDBenfName) || FipaUtils.nullOrBlank(strCDBenfNRIC) ||
	//			FipaUtils.nullOrBlank(strCDBenfIncorpNo) || FipaUtils.nullOrBlank(strCDBenfRegAddr)||
	//			FipaUtils.nullOrBlank(strCDBenfWork) || FipaUtils.nullOrBlank(strCDBenfWrkCountry) || 
	//			FipaUtils.nullOrBlank(strCDBenfRelation) || FipaUtils.nullOrBlank(strCDBenfRegAddr2) || 
	//			FipaUtils.nullOrBlank(strCDBenfPostal) || FipaUtils.nullOrBlank(strCDBenfCntry))){ 
	//		 
	...
	//	fnaselfspsDets.setFnaId(strFNAId);
	//	 
	//	String strCDPEPName = request.getParameter("amlPepName") != null ? request.getParameter("amlPepName") : null;
	//	fnaOthPerDets.setAmlBenfName(strCDPEPName);
	//	
	//	String strCDPEPNRIC = request.getParameter("amlPepNric") != null ? request.getParameter("amlPepNric") : null;
	//	fnaOthPerDets.setAmlBenfNric(strCDPEPNRIC);
	//	
	//	String strCDPEPIncorpNo = request.getParameter("amlPepIncno") != null ? request.getParameter("amlPepIncno") : null;
	//	fnaOthPerDets.setAmlBenfIncno(strCDPEPIncorpNo);
	//	
	//	String strCDPEPRegAddr = request.getParameter("amlPepAddr1") != null ? request.getParameter("amlPepAddr1") : null;
	//	fnaOthPerDets.setAmlBenfAddr1(strCDPEPRegAddr);
	//	
	//	String strCDPEPWork = request.getParameter("amlPepJob") != null ? request.getParameter("amlPepJob") : null;
	//	fnaOthPerDets.setAmlBenfJob(strCDPEPWork);
	//	
	//	String strCDPEPWrkCountry = request.getParameter("amlPepJobconty") != null ? request.getParameter("amlPepJobconty") : null;
	//	fnaOthPerDets.setAmlBenfJobconty(strCDPEPWrkCountry);
	//	
	//	String strCDPEPRelation = request.getParameter("amlPepRelat") != null ? request.getParameter("amlPepRelat") : null;
	//	fnaOthPerDets.setAmlBenfRelat(strCDPEPRelation);
	//	
	//	String strCDPEPRegAddr2 = request.getParameter("amlPepAddr2") != null ? request.getParameter("amlPepAddr2") : null;
	//	fnaOthPerDets.setAmlBenfAddr2(strCDPEPRegAddr2);
	//	String strCDPEPPostal = request.getParameter("amlPepPostal") != null ? request.getParameter("amlPepPostal") : null;
	//	fnaOthPerDets.setAmlBenfPostal(strCDPEPPostal); 
	//	String strCDPEPCntry = request.getParameter("amlPepConty") != null ? request.getParameter("amlPepConty") : null;
	//	fnaOthPerDets.setAmlBenfConty(strCDPEPCntry);
	//	
	//	if(!(FipaUtils.nullOrBlank(strCDPEPName) || FipaUtils.nullOrBlank(strCDPEPNRIC) ||
	//			FipaUtils.nullOrBlank(strCDPEPIncorpNo) || FipaUtils.nullOrBlank(strCDPEPRegAddr) ||
	//			FipaUtils.nullOrBlank(strCDPEPWork) || FipaUtils.nullOrBlank(strCDPEPWrkCountry)  ||
	//			FipaUtils.nullOrBlank(strCDPEPRelation) | FipaUtils.nullOrBlank(strCDPEPRegAddr2)  || 
	//			FipaUtils.nullOrBlank(strCDPEPPostal) || FipaUtils.nullOrBlank(strCDPEPCntry))){
	//		 
	...
	//	
	//	
	//	String strCDTPPName = request.getParameter("amlTppName") != null ? request.getParameter("amlTppName") : null;
	//	fnaOthPerDets.setTxtFldOthPerName(strCDTPPName);
	//	
	//	String strCDTPPNRIC = request.getParameter("amlTppNric") != null ? request.getParameter("amlTppNric") : null;
	//	fnaOthPerDets.setTxtFldOthPerNric(strCDTPPNRIC);
	//	
	//	String strCDTPPIncorpNo = request.getParameter("amlTppIncno") != null ? request.getParameter("amlTppIncno") : null;
	//	fnaOthPerDets.setTxtFldOthPerIncorpno(strCDTPPIncorpNo);
	//	
	//	String strCDTPPRegAddr = request.getParameter("amlTppAddr1") != null ? request.getParameter("amlTppAddr1") : null;
	//	fnaOthPerDets.setTxtFldOthPerRegAddr(strCDTPPRegAddr);
	//	
	//	String strCDTPPWork = request.getParameter("amlTppJob") != null ? request.getParameter("amlTppJob") : null;
	//	fnaOthPerDets.setTxtFldOthPerWork(strCDTPPWork);
	//	
	//	String strCDTPPWrkCountry = request.getParameter("amlTppJobconty") != null ? request.getParameter("amlTppJobconty") : null;
	//	fnaOthPerDets.setTxtFldOthPerWorkCountry(strCDTPPWrkCountry);
	//	
	//	String strCDTPPRelation = request.getParameter("amlTppRelat") != null ? request.getParameter("amlTppRelat") : null;
	//	fnaOthPerDets.setTxtFldOthPerRelation(strCDTPPRelation);
	//	
	//	String strOthPerPayMode = request.getParameter("amlTppPaymode") != null ? request.getParameter("amlTppPaymode") : null;
	//	fnaOthPerDets.setTxtFldOthPerPayMode(strOthPerPayMode);
	//	
	//	String strCDTPPBankName = request.getParameter("amlTppBank") != null ? request.getParameter("amlTppBank") : null;
	//	fnaOthPerDets.setTxtFldOthPerBankName(strCDTPPBankName);
	//	
	//	String strCDTPPChqOrderNo = request.getParameter("amlTppChqno") != null ? request.getParameter("amlTppChqno") : null;
	//	fnaOthPerDets.setTxtFldOthPerChqOrderNo(strCDTPPChqOrderNo);
	//	
	//	String strCDTPPContNo = request.getParameter("amlTppMobile") != null ? request.getParameter("amlTppMobile") : null;
	//	fnaOthPerDets.setam (strCDTPPContNo);
	//	
	//	String strCDTPPRegAddr2 = request.getParameter("amlTppAddr2") != null ? request.getParameter("amlTppAddr2") : null;
	//	fnaOthPerDets.setAmlBenfAddr2(strCDTPPRegAddr2);
	//	String strCDTPPPostal = request.getParameter("amlTppPostal") != null ? request.getParameter("amlTppPostal") : null;
	//	fnaOthPerDets.setAmlBenfPostal(strCDTPPPostal); 
	//	String strCDTPPCntry = request.getParameter("amlTppConty") != null ? request.getParameter("amlTppConty") : null;
	//	fnaOthPerDets.setAmlBenfConty(strCDTPPCntry);
	//	
	//	if(!(FipaUtils.nullOrBlank(strCDTPPName) || FipaUtils.nullOrBlank(strCDTPPNRIC) ||
	//			FipaUtils.nullOrBlank(strCDTPPIncorpNo) || FipaUtils.nullOrBlank(strCDTPPRegAddr) ||
	//			FipaUtils.nullOrBlank(strCDTPPWork) || FipaUtils.nullOrBlank(strCDTPPWrkCountry)  ||
	//			FipaUtils.nullOrBlank(strCDTPPRelation) || FipaUtils.nullOrBlank(strOthPerPayMode) ||
	//			FipaUtils.nullOrBlank(strCDTPPBankName) || FipaUtils.nullOrBlank(strCDTPPChqOrderNo)  ||
	//			 FipaUtils.nullOrBlank(strCDTPPContNo)  || FipaUtils.nullOrBlank(strCDTPPRegAddr2)  || 
	...
	public Vector getCashOfBankDetails(HttpServletRequest request) {
		...
		List updCobList = new ArrayList();List updCobListAS = new ArrayList();
		...
		if(strArrCobMode != null && strArrCobMode.length>0){
			...
			for(int id=0;id<strArrCobMode.length;id++){
				...
				double dblCobCurBalc=Double.parseDouble(FipaUtils.nullOrBlank(strArrCobCurBalance[id]) ? "0" :strArrCobCurBalance[id]);
				double dblCobRegDep =Double.parseDouble(FipaUtils.nullOrBlank(strArrCobRegDeposit[id]) ? "0" :strArrCobRegDeposit[id]);
				double dblCobRetrmntPrcnt=Double.parseDouble(FipaUtils.nullOrBlank(strArrCobRetrmtPrcnt[id]) ? "0" :strArrCobRetrmtPrcnt[id]);
				 
				Date DateCobPerFrom = FipaUtils.nullOrBlank(strArrCobPerFrom[id]) ? null:FipaDateUtils.convertStringToDate(strArrCobPerFrom[id]);
				Date DateCobPerTo = FipaUtils.nullOrBlank(strArrCobPerTo[id]) ?  null :FipaDateUtils.convertStringToDate(strArrCobPerTo[id]);
				
				
				
				Date crtdDate = FipaUtils.nullOrBlank(strArrCobCrtdDate[id]) ? new Date() :FipaDateUtils.convertStringToDate(strArrCobCrtdDate[id]);
				...
				if(strArrCobMode[id].equalsIgnoreCase(FipaConstant.INSERT_MODE) && FipaUtils.nullOrBlank(strPKId)){
					insCobList.add(new FnaCashAtBank(null,null,strArrCobMainHoldrName[id],strArrCobSuppHoldrName[id],strArrCobRelationship[id],strArrCobOwnershipType[id],strArrCobBankName[id],strArrCobBankAccNo[id],strArrCobAccountType[id],dblCobCurBalc,dblCobRegDep,strArrCobDepositFreq[id],DateCobPerFrom,DateCobPerTo,strArrCobObjective[id],dblCobRetrmntPrcnt,strArrCobChildName[id],strArrCobRemarks[id],strLoggedUser,new Date(),null,null,strArrCobRefId[id]));
			 
				 }
				
				if(strArrCobMode[id].equalsIgnoreCase(FipaConstant.UPDATE_MODE) || 
						(strArrCobMode[id].equalsIgnoreCase(FipaConstant.INSERT_MODE) && !FipaUtils.nullOrBlank(strPKId))){
					if(strPKId.startsWith("AS")){
						updCobListAS.add(new FnaCashAtBank(strArrCashBankId[id],fnaselfspsDets,strArrCobMainHoldrName[id],strArrCobSuppHoldrName[id],strArrCobRelationship[id],strArrCobOwnershipType[id],strArrCobBankName[id],strArrCobBankAccNo[id],strArrCobAccountType[id],dblCobCurBalc,dblCobRegDep,strArrCobDepositFreq[id],DateCobPerFrom,DateCobPerTo,strArrCobObjective[id],dblCobRetrmntPrcnt,strArrCobChildName[id],strArrCobRemarks[id],strLoggedUser,new Date(),strLoggedUser,new Date(),strArrCobRefId[id]));
						 
					}else{
						updCobList.add(new FnaCashAtBank(strArrCashBankId[id],fnaselfspsDets,strArrCobMainHoldrName[id],strArrCobSuppHoldrName[id],strArrCobRelationship[id],strArrCobOwnershipType[id],strArrCobBankName[id],strArrCobBankAccNo[id],strArrCobAccountType[id],dblCobCurBalc,dblCobRegDep,strArrCobDepositFreq[id],DateCobPerFrom,DateCobPerTo,strArrCobObjective[id],dblCobRetrmntPrcnt,strArrCobChildName[id],strArrCobRemarks[id],strArrCobCrtdBy[id],crtdDate,strLoggedUser,new Date(),strArrCobRefId[id]));
				...
				if(strArrCobMode[id].equalsIgnoreCase(FipaConstant.DELETE_MODE)){
					delCobList.add(new FnaCashAtBank(strArrCashBankId[id],fnaselfspsDets,strArrCobMainHoldrName[id],strArrCobSuppHoldrName[id],strArrCobRelationship[id],strArrCobOwnershipType[id],strArrCobBankName[id],strArrCobBankAccNo[id],strArrCobAccountType[id],dblCobCurBalc,dblCobRegDep,strArrCobDepositFreq[id],DateCobPerFrom,DateCobPerTo,strArrCobObjective[id],dblCobRetrmntPrcnt,strArrCobChildName[id],strArrCobRemarks[id],strArrCobCrtdBy[id],crtdDate,strLoggedUser,new Date(),strArrCobRefId[id]));
		...
		vectCobDetails.add(delCobList);vectCobDetails.add(updCobListAS);
	 
		}catch(Exception ex){
			logger.debug("Error in getCashOfBankDetails ->" , ex);
		}
	...
	public Vector getSRSDetails(HttpServletRequest request) {
		...
		List updSRSList = new ArrayList();List updSRSListAS = new ArrayList();
		...
		if(strArrSRSMode != null && strArrSRSMode.length>0){
			...
			for(int id=0;id<strArrSRSMode.length;id++){
				...
				double dblSrsAmount		=Double.parseDouble(FipaUtils.nullOrBlank(strArrSrsAmount[id]) ? "0" :strArrSrsAmount[id]);
				  
				Date DateSrsPerdFrom = FipaUtils.nullOrBlank(strArrSrsPerdFrom[id]) ? null:FipaDateUtils.convertStringToDate(strArrSrsPerdFrom[id]);
				Date DateSrsPerdTo = FipaUtils.nullOrBlank(strArrSrsPerdTo[id]) ?  null :FipaDateUtils.convertStringToDate(strArrSrsPerdTo[id]);
				
				
				
				Date crtdDate = FipaUtils.nullOrBlank(strArrSrsCrtdDate[id]) ? new Date() :FipaDateUtils.convertStringToDate(strArrSrsCrtdDate[id]);
				...
				if(strArrSRSMode[id].equalsIgnoreCase(FipaConstant.INSERT_MODE)&& FipaUtils.nullOrBlank(strPKId)){
					insSRSList.add(new FnaSrs(null,null,strArrSRSRefId[id],strArrSrsClassify[id],strArrSrsDesc[id],strArrSrsFreq[id],dblSrsAmount,DateSrsPerdFrom,DateSrsPerdTo,strArrSrsAgestart[id],strArrSrsPayoutPerd[id],strLoggedUser,new Date(),null,null));
			 
				 }
				
				if(strArrSRSMode[id].equalsIgnoreCase(FipaConstant.UPDATE_MODE)|| 
						(strArrSRSMode[id].equalsIgnoreCase(FipaConstant.INSERT_MODE) && !FipaUtils.nullOrBlank(strPKId))){
					
					if(strPKId.startsWith("AS")){
						updSRSListAS.add(new FnaSrs(strArrSRSId[id],fnaselfspsDets,strArrSRSRefId[id],strArrSrsClassify[id],strArrSrsDesc[id],strArrSrsFreq[id],dblSrsAmount,DateSrsPerdFrom,DateSrsPerdTo,strArrSrsAgestart[id],strArrSrsPayoutPerd[id],strLoggedUser,new Date(),strLoggedUser,new Date()));
						 
					}else{
						updSRSList.add(new FnaSrs(strArrSRSId[id],fnaselfspsDets,strArrSRSRefId[id],strArrSrsClassify[id],strArrSrsDesc[id],strArrSrsFreq[id],dblSrsAmount,DateSrsPerdFrom,DateSrsPerdTo,strArrSrsAgestart[id],strArrSrsPayoutPerd[id],strArrSrsCrtdBy[id],crtdDate,strLoggedUser,new Date()));
				...
				if(strArrSRSMode[id].equalsIgnoreCase(FipaConstant.DELETE_MODE)){
					delSRSList.add(new FnaSrs(strArrSRSId[id],fnaselfspsDets,strArrSRSRefId[id],strArrSrsClassify[id],strArrSrsDesc[id],strArrSrsFreq[id],dblSrsAmount,DateSrsPerdFrom,DateSrsPerdTo,strArrSrsAgestart[id],strArrSrsPayoutPerd[id],strArrSrsCrtdBy[id],crtdDate,strLoggedUser,new Date()));
		...
		vectSRSDetails.add(delSRSList);vectSRSDetails.add(updSRSListAS);
		}catch(Exception ex){
			logger.debug("Error in getSRSDetails ->" , ex);
		}
	...
	public Vector getliMvRetDetails(HttpServletRequest request) {
		...
		List updliRetPlgList = new ArrayList();List updliRetPlgListAS = new ArrayList();
	...
	//	String[] strArrLiIncomeStrm 		= FipaUtils.getParamArrValue(request, "txtFldIncomeStream"); 
	
	//	String[] strArrLiPvFvOnRet		= FipaUtils.getParamArrValue(request, "txtFldPvFvOnRet");
		...
		if(strArrRetPlgMode != null && strArrRetPlgMode.length>0){
			...
			for(int id=0;id<strArrRetPlgMode.length;id++){
				...
				double dbllipGtdInc=  Double.parseDouble(FipaUtils.nullOrBlank(strArrLiGtdIncome[id]) ? "0" :strArrLiGtdIncome[id]);
				double dbllipNGtdInc =Double.parseDouble(FipaUtils.nullOrBlank(strArrLiNGtdIncome[id]) ? "0" :strArrLiNGtdIncome[id]);
	//			double dbllipIncStrm = Double.parseDouble(FipaUtils.nullOrBlank(strArrLiIncomeStrm[id]) ? "0" :strArrLiIncomeStrm[id]);
				double dbllipIncStrm =0;
	//			double dbllipPvFvOnRet = Double.parseDouble(FipaUtils.nullOrBlank(strArrLiPvFvOnRet[id]) ? "0" :strArrLiPvFvOnRet[id]); 
				double dbllipPvFvOnRet=0;
				double dbllipAssBankInt = Double.parseDouble(FipaUtils.nullOrBlank(strArrLiAssBankInt[id]) ? "0" :strArrLiAssBankInt[id]);
				 
				
				double dbllipTotIncome = Double.parseDouble(FipaUtils.nullOrBlank(strArrLiTotalIncome[id]) ? "0" :strArrLiTotalIncome[id]);
			 
				
				double dbllipEsclRate=Double.parseDouble(FipaUtils.nullOrBlank(strArrLiEsclRate[id]) ? "0" :strArrLiEsclRate[id]);
				 
				Date crtdDate = FipaUtils.nullOrBlank(strArrMvRetCrtdDate[id]) ? new Date() :FipaDateUtils.convertStringToDate(strArrMvRetCrtdDate[id]);
				
				
				Date incDate = FipaUtils.nullOrBlank(strArrMvRetIncDate[id]) ? new Date() :FipaDateUtils.convertStringToDate(strArrMvRetIncDate[id]);
				Date expDate = FipaUtils.nullOrBlank(strArrMvRetExpDate[id]) ? new Date() :FipaDateUtils.convertStringToDate(strArrMvRetExpDate[id]);
				...
				if(strArrRetPlgMode[id].equalsIgnoreCase(FipaConstant.INSERT_MODE) && FipaUtils.nullOrBlank(strPKId)){
					insliRetPlgList.add(new FnaLifeinsuranceMvRet(null,fnalifeInsDets,null,strArrLiComceAge[id],strArrLiEndAge[id],dbllipEsclRate,dbllipGtdInc,dbllipNGtdInc,dbllipIncStrm,dbllipPvFvOnRet,dbllipAssBankInt,dbllipTotIncome,strArrMvLiRemarks[id],strLoggedUser,new Date(),null,null,strArrMvRetRefId[id],strArrMvRetPlanName[id],incDate,expDate));
	
				 }
				
				if(strArrRetPlgMode[id].equalsIgnoreCase(FipaConstant.UPDATE_MODE)|| 
						(strArrRetPlgMode[id].equalsIgnoreCase(FipaConstant.INSERT_MODE) && !FipaUtils.nullOrBlank(strPKId))){
					
					if(strPKId.startsWith("AS")){
						updliRetPlgListAS.add(new FnaLifeinsuranceMvRet(strArrRetPlgId[id],fnalifeInsDets,fnaselfspsDets,strArrLiComceAge[id],strArrLiEndAge[id],dbllipEsclRate,dbllipGtdInc,dbllipNGtdInc,dbllipIncStrm,dbllipPvFvOnRet,dbllipAssBankInt,dbllipTotIncome,strArrMvLiRemarks[id],strLoggedUser,new Date(),strLoggedUser,new Date(),strArrMvRetRefId[id],strArrMvRetPlanName[id],incDate,expDate)); 
						 
					}else{
						updliRetPlgList.add(new FnaLifeinsuranceMvRet(strArrRetPlgId[id],fnalifeInsDets,fnaselfspsDets,strArrLiComceAge[id],strArrLiEndAge[id],dbllipEsclRate,dbllipGtdInc,dbllipNGtdInc,dbllipIncStrm,dbllipPvFvOnRet,dbllipAssBankInt,dbllipTotIncome,strArrMvLiRemarks[id],strArrMvRetCrtdBy[id],crtdDate,strLoggedUser,new Date(),strArrMvRetRefId[id],strArrMvRetPlanName[id],incDate,expDate)); 
				...
				if(strArrRetPlgMode[id].equalsIgnoreCase(FipaConstant.DELETE_MODE)){
					delliRetPlgList.add(new FnaLifeinsuranceMvRet(strArrRetPlgId[id],fnalifeInsDets,fnaselfspsDets,strArrLiComceAge[id],strArrLiEndAge[id],dbllipEsclRate,dbllipGtdInc,dbllipNGtdInc,dbllipIncStrm,dbllipPvFvOnRet,dbllipAssBankInt,dbllipTotIncome,strArrMvLiRemarks[id],strArrMvRetCrtdBy[id],crtdDate,strLoggedUser,new Date(),strArrMvRetRefId[id],strArrMvRetPlanName[id],incDate,expDate)); 
		...
		vectliRetPlgDet.add(delliRetPlgList);vectliRetPlgDet.add(updliRetPlgListAS);
		}catch(Exception ex){
			logger.debug("Error in getliMvRetDetails ->" , ex);
		}
...
//		System.out.println("strArrLiChlBegAge.length--->"+strArrLiChlBegAge.length+",strArrEduPlgMode.length-->"+strArrEduPlgMode.length);
		...
		if(strArrEduPlgMode != null && strArrEduPlgMode.length>0){
			...
			for(int id=0;id<strArrEduPlgMode.length;id++){
				...
				double dbllipBankIntRate =  Double.parseDouble(FipaUtils.nullOrBlank(strArrLiBankIntRate[id]) ? "0" :strArrLiBankIntRate[id]);
				double dbllipInflationRate =Double.parseDouble(FipaUtils.nullOrBlank(strArrLiInfRate[id]) ? "0" :strArrLiInfRate[id]);
				double dbllipProPaid= Double.parseDouble(FipaUtils.nullOrBlank(strArrLiTotProPaid[id]) ? "0" :strArrLiTotProPaid[id]);
				double dbllipChldAge =  Double.parseDouble(FipaUtils.nullOrBlank(strArrLiChldTerAge[id]) ? "0" :strArrLiChldTerAge[id]);
				
				double dbllipLoanIntRate =  Double.parseDouble(FipaUtils.nullOrBlank(strArrLiLoanIntRate[id]) ? "0" :strArrLiLoanIntRate[id]);
				
				
				 
				Date incDate = FipaUtils.nullOrBlank(strArrLiIncDate[id]) ? null :FipaDateUtils.convertStringToDate(strArrLiIncDate[id]);
				Date expDate = FipaUtils.nullOrBlank(strArrLiExpDate[id]) ? null :FipaDateUtils.convertStringToDate(strArrLiExpDate[id]);
...
//				Date crtdDate = FipaUtils.nullOrBlank(strArrEduPlgCrtdDate[id]) ? new Date() :FipaDateUtils.convertStringToDate(strArrEduPlgCrtdDate[id]);
				...
				if(strArrEduPlgMode[id].equalsIgnoreCase(FipaConstant.INSERT_MODE) && FipaUtils.nullOrBlank(strPKId) ){
					insliEduPlgList.add(new FnaLifeinsuranceChildedc(null,null,null, strArrLiChldname[id],strArrLiChldAge[id],strArrLiTerEduAge[id], dbllipBankIntRate,dbllipLoanIntRate, dbllipInflationRate,strArrLiChlBegAge[id],dbllipProPaid, dbllipChldAge,null,strLoggedUser,new Date(), null, null,strArrLiRefId[id],strArrLiPlanName[id],incDate,expDate)); 
				 }
				
				if(strArrEduPlgMode[id].equalsIgnoreCase(FipaConstant.UPDATE_MODE)|| 
						(strArrEduPlgMode[id].equalsIgnoreCase(FipaConstant.INSERT_MODE) && !FipaUtils.nullOrBlank(strPKId))){
					
					if(strPKId.startsWith("AS")){
						updliEduPlgListAS.add(new FnaLifeinsuranceChildedc(strArrEduPlgId[id],fnalifeInsDets,fnaselfspsDets, strArrLiChldname[id],strArrLiChldAge[id],strArrLiTerEduAge[id], dbllipBankIntRate, dbllipLoanIntRate,dbllipInflationRate,strArrLiChlBegAge[id],dbllipProPaid, dbllipChldAge,null,strLoggedUser,new Date(),strLoggedUser,new Date(),strArrLiRefId[id],strArrLiPlanName[id],incDate,expDate)); 
						 
					}else{
						updliEduPlgList.add(new FnaLifeinsuranceChildedc(strArrEduPlgId[id],fnalifeInsDets,fnaselfspsDets, strArrLiChldname[id],strArrLiChldAge[id],strArrLiTerEduAge[id], dbllipBankIntRate, dbllipLoanIntRate,dbllipInflationRate,strArrLiChlBegAge[id],dbllipProPaid, dbllipChldAge,null,strLoggedUser,new Date(),strLoggedUser,new Date(),strArrLiRefId[id],strArrLiPlanName[id],incDate,expDate)); 
				...
				if(strArrEduPlgMode[id].equalsIgnoreCase(FipaConstant.DELETE_MODE)){
					delliEduPlgList.add(new FnaLifeinsuranceChildedc(strArrEduPlgId[id],fnalifeInsDets,fnaselfspsDets, strArrLiChldname[id],strArrLiChldAge[id],strArrLiTerEduAge[id], dbllipBankIntRate,dbllipLoanIntRate, dbllipInflationRate,strArrLiChlBegAge[id],dbllipProPaid, dbllipChldAge,null,strLoggedUser,new Date(),strLoggedUser,new Date(),strArrLiRefId[id],strArrLiPlanName[id],incDate,expDate)); 
		 
				}
			}
		}
//		System.out.println(updliEduPlgListAS);
		
		vectliEduPlgDet.add(insliEduPlgList);
		vectliEduPlgDet.add(updliEduPlgList);
		vectliEduPlgDet.add(delliEduPlgList);	vectliEduPlgDet.add(updliEduPlgListAS);
		}catch(Exception ex){
			logger.debug("Error in getliChildDets ->" , ex);
			ex.printStackTrace();
		}
...
//		vectliPlnProDet.add(delliPlnProList);vectliPlnProDet.add(updliPlnProListAS);
//		}catch(Exception ex){
//			logger.debug("Error in getliPlnProDetails ->" , ex);
//		}
//		
//		
...
//		System.out.println("getliPlnProDetails ----------->");
		...
		if(strArrPlnProMode != null && strArrPlnProMode.length>0){ 
			...
			for(int id=0;id<strArrPlnProMode.length;id++){ 
				...
				double dblInsCashVal =Double.parseDouble(FipaUtils.nullOrBlank(strArrPlnInsCashVal[id]) ? "0" :strArrPlnInsCashVal[id]);
				double dblPlnLifeInsLoans =Double.parseDouble(FipaUtils.nullOrBlank(strArrPlnLifeInsLoans[id]) ? "0" :strArrPlnLifeInsLoans[id]);
				
				
				Date crtdDate = FipaUtils.nullOrBlank(strArrPlnProCtrtDate[id]) ? new Date() :FipaDateUtils.convertStringToDate(strArrPlnProCtrtDate[id]);
				  
				Date incDate =  FipaUtils.nullOrBlank(strArrPlnProIncepDate[id]) ? null :FipaDateUtils.convertStringToDate(strArrPlnProIncepDate[id]);
				Date expDate =  FipaUtils.nullOrBlank(strArrPlnProExpDate[id]) ? null :FipaDateUtils.convertStringToDate(strArrPlnProExpDate[id]);
				//Date premTrDate =FipaUtils.nullOrBlank(strArrPlnProPremTrmDate[id]) ? null :FipaDateUtils.convertStringToDate(strArrPlnProPremTrmDate[id]);
		...
		if(!FipaUtils.nullOrBlank(strArrPlnProName[id])){
				...
				if(strArrPlnProMode[id].equalsIgnoreCase(FipaConstant.INSERT_MODE) && FipaUtils.nullOrBlank(strPKId) ){ 
					insliPlnProList.add(new FnaLifeinsuranceBasicriders(null,null,null, strArrPlnProName[id],strArrPlnProType[id],strArrPlnProPremTrm[id], dbllipSA,dbllipPremAmt, strArrPlnProPayMode[id], strArrPlnProMtd[id],strArrPlnProCov[id], strArrPlnProRmks[id], strLoggedUser,new Date(), null, null,expDate,incDate,strArrPlnRefId[id],strArrBasicRefId[id],intbplfretYrstoret,intbpretSelfretage,intbpretSpouseretage,strArrbpretMultionret[id],dblbpretCashvalonret,dblbpretIntrateused,dblbpretPrcnttoused,intbpcashvalRetAge,strArrbpcashvalRefId[id],premTrDate,dblInsCashVal,dblPlnLifeInsLoans));
				...
				if(strArrPlnProMode[id].equalsIgnoreCase(FipaConstant.UPDATE_MODE)|| 
					...
					if(strPKId.startsWith("AS")){
						updliPlnProListAS.add(new FnaLifeinsuranceBasicriders(strArrPlnProId[id],fnalifeInsDets,fnaselfspsDets, strArrPlnProName[id],strArrPlnProType[id],strArrPlnProPremTrm[id], dbllipSA,dbllipPremAmt, strArrPlnProPayMode[id], strArrPlnProMtd[id],strArrPlnProCov[id], strArrPlnProRmks[id],strLoggedUser,new Date(),strLoggedUser,new Date(),expDate,incDate,strArrPlnRefId[id],strArrBasicRefId[id],intbplfretYrstoret,intbpretSelfretage,intbpretSpouseretage,strArrbpretMultionret[id],dblbpretCashvalonret,dblbpretIntrateused,dblbpretPrcnttoused,intbpcashvalRetAge,strArrbpcashvalRefId[id],premTrDate,dblInsCashVal,dblPlnLifeInsLoans)); 
						 
					}else{
						updliPlnProList.add(new FnaLifeinsuranceBasicriders(strArrPlnProId[id],fnalifeInsDets,fnaselfspsDets, strArrPlnProName[id],strArrPlnProType[id],strArrPlnProPremTrm[id], dbllipSA,dbllipPremAmt, strArrPlnProPayMode[id], strArrPlnProMtd[id],strArrPlnProCov[id], strArrPlnProRmks[id],strArrPlnProCrtdBy[id],crtdDate,strLoggedUser,new Date(),expDate,incDate,strArrPlnRefId[id],strArrBasicRefId[id],intbplfretYrstoret,intbpretSelfretage,intbpretSpouseretage,strArrbpretMultionret[id],dblbpretCashvalonret,dblbpretIntrateused,dblbpretPrcnttoused,intbpcashvalRetAge,strArrbpcashvalRefId[id],premTrDate,dblInsCashVal,dblPlnLifeInsLoans)); 
...
//		String strFNAId = FipaUtils.getParamValue(request, "fnaId");
		...
		String strEstTrstId = request.getParameter("txtFldEstTrstId") != null ? request.getParameter("txtFldEstTrstId") : null;
		fnaEstPlnDets.setEstId(strEstTrstId);
		String strEstTrstDesc = request.getParameter("txtFldEstTrstDesc") != null ? request.getParameter("txtFldEstTrstDesc") : null;
		fnaEstPlnDets.setEstPlanDesc(strEstTrstDesc);
		String strEstSlfTrstFlg = request.getParameter("txtFldEstSlfTrstFlg") != null ? request.getParameter("txtFldEstSlfTrstFlg") : null;
		fnaEstPlnDets.setEstSelfPlanFlg(strEstSlfTrstFlg);
		String strEstSpsTrstFlg = request.getParameter("txtFldEstSpsTrstFlg") != null ? request.getParameter("txtFldEstSpsTrstFlg") : null;
		fnaEstPlnDets.setEstSpsPlanFlg(strEstSpsTrstFlg);
		String strEstTrstRmrks = request.getParameter("txtFldEstTrstRmrks") != null ? request.getParameter("txtFldEstTrstRmrks") : null;
		...
		String strEstLPOAId = request.getParameter("txtFldEstLPOAId") != null ? request.getParameter("txtFldEstLPOAId") : null;
		fnaEstPlnDets.setEstId(strEstLPOAId);
		
		String strEstLPOADesc = request.getParameter("txtFldEstLPOADesc") != null ? request.getParameter("txtFldEstLPOADesc") : null;
		fnaEstPlnDets.setEstPlanDesc(strEstLPOADesc); 
		String strEstSlfLPOAFlg = request.getParameter("txtFldEstSlfLPOAFlg") != null ? request.getParameter("txtFldEstSlfLPOAFlg") : null;
		fnaEstPlnDets.setEstSelfPlanFlg(strEstSlfLPOAFlg);
		String strEstSpsLPOAFlg = request.getParameter("txtFldEstSpsLPOAFlg") != null ? request.getParameter("txtFldEstSpsLPOAFlg") : null;
		fnaEstPlnDets.setEstSpsPlanFlg(strEstSpsLPOAFlg);
		String strEstLPOARmrks = request.getParameter("txtFldEstLPOARmrks") != null ? request.getParameter("txtFldEstLPOARmrks") : null;
	...
	//			FipaUtils.nullOrBlank(strEstSpsLPOAFlg) || FipaUtils.nullOrBlank(strEstLPOARmrks))){
		...
		String strEstCharityId = request.getParameter("txtFldEstCharityId") != null ? request.getParameter("txtFldEstCharityId") : null;
		fnaEstPlnDets.setEstId(strEstCharityId);
		
		String strEstCharityDesc = request.getParameter("txtFldEstCharityDesc") != null ? request.getParameter("txtFldEstCharityDesc") : null;
		fnaEstPlnDets.setEstPlanDesc(strEstCharityDesc); 
		String strEstSlfCharityFlg = request.getParameter("txtFldEstSlfCharityFlg") != null ? request.getParameter("txtFldEstSlfCharityFlg") : null;
		fnaEstPlnDets.setEstSelfPlanFlg(strEstSlfCharityFlg);
		String strEstSpsCharityFlg = request.getParameter("txtFldEstSpsCharityFlg") != null ? request.getParameter("txtFldEstSpsCharityFlg") : null;
		fnaEstPlnDets.setEstSpsPlanFlg(strEstSpsCharityFlg); 
		String strEstCharityRmrks = request.getParameter("txtFldEstCharityRmrks") != null ? request.getParameter("txtFldEstCharityRmrks") : null;
	...
	//			FipaUtils.nullOrBlank(strEstSpsCharityFlg) || FipaUtils.nullOrBlank(strEstCharityRmrks))){
		...
		String strEstOverseasId = request.getParameter("txtFldEstOverseasId") != null ? request.getParameter("txtFldEstOverseasId") : null;
		fnaEstPlnDets.setEstId(strEstOverseasId); 
		String strEstOverseasDesc = request.getParameter("txtFldEstOverseasDesc") != null ? request.getParameter("txtFldEstOverseasDesc") : null;
		fnaEstPlnDets.setEstPlanDesc(strEstOverseasDesc); 
		String strEstSlfOverseasFlg = request.getParameter("txtFldEstSlfOverseasFlg") != null ? request.getParameter("txtFldEstSlfOverseasFlg") : null;
		fnaEstPlnDets.setEstSelfPlanFlg(strEstSlfOverseasFlg);
		String strEstSpsOverseasFlg = request.getParameter("txtFldEstSpsOverseasFlg") != null ? request.getParameter("txtFldEstSpsOverseasFlg") : null;
		fnaEstPlnDets.setEstSpsPlanFlg(strEstSpsOverseasFlg);   
		String strEstOverseasRmrks = request.getParameter("txtFldEstOverseasRmrks") != null ? request.getParameter("txtFldEstOverseasRmrks") : null;
	...
	//			FipaUtils.nullOrBlank(strEstSpsOverseasFlg) || FipaUtils.nullOrBlank(strEstOverseasRmrks))){
		...
		vectEstPlnDet.add(estPlnList); 
			...
			logger.debug("Error in getEstPlanDetails ->" , ex);
		}
...
//			String strFNAId     				= FipaUtils.getParamValue(request, "fnaId");
			...
			if(strArrCovMode != null && strArrCovMode.length>0){
				...
				for(int id=0;id<strArrCovMode.length;id++){
					...
					Double dlbBoostr=Double.parseDouble(FipaUtils.nullOrBlank(strArrCovBoostr[id]) ? "0" :strArrCovBoostr[id]);
					Double dlbAmtAss=Double.parseDouble(FipaUtils.nullOrBlank(strArrCovAmtAss[id]) ? "0" :strArrCovAmtAss[id]);
					Double dlbDsbenf=Double.parseDouble(FipaUtils.nullOrBlank(strArrCovDsbltyBenf[id]) ? "0" :strArrCovDsbltyBenf[id]);
					Double dlbDeduct=Double.parseDouble(FipaUtils.nullOrBlank(strArrCovDeductible[id]) ? "0" :strArrCovDeductible[id]);
					Double dlbCoIns=Double.parseDouble(FipaUtils.nullOrBlank(strArrCovInsurance[id]) ? "0" :strArrCovInsurance[id]);
					 
					 
					 
					Date crtdDate = FipaUtils.nullOrBlank(strArrDsbCrtdDate[id]) ? new Date() :FipaDateUtils.convertStringToDate(strArrDsbCrtdDate[id]);
					
					Date incDate = FipaUtils.nullOrBlank(strArrCovIncDate[id]) ? null :FipaDateUtils.convertStringToDate(strArrCovIncDate[id]);
					Date expDate = FipaUtils.nullOrBlank(strArrCovExpDate[id]) ? null :FipaDateUtils.convertStringToDate(strArrCovExpDate[id]);
					...
					if(strArrCovMode[id].equalsIgnoreCase(FipaConstant.INSERT_MODE) && FipaUtils.nullOrBlank(strPKId) ){
						insliCovList.add(new FnaLifeinsuranceCoverages(null,null,null,strArrCovTypeofcov[id],strArrliPlanPKId[id],strArrCovPlanName[id],strArrCovBasicRiderRef[id],incDate,expDate,null,null,null,null,dlbDeduct,dlbCoIns,strArrCovRemarks[id],strLoggedUser,new Date(),null,null,dlbBoostr,dlbAmtAss,strArrCovExpPrd[id],strArrCovNoofyrs[id],dlbDsbenf,strArrCovYrsofpay[id],strArrCovMaxpayout[id],strArrCovTypesofHS[id],strArrCovDescription[id]));
		 			
					 }
					
					if(strArrCovMode[id].equalsIgnoreCase(FipaConstant.UPDATE_MODE)  || 
							(strArrCovMode[id].equalsIgnoreCase(FipaConstant.INSERT_MODE) && !FipaUtils.nullOrBlank(strPKId))){
						if(strPKId.startsWith("AS")){
//							updliCovListAS.add(new FnaLifeinsuranceCoverages(strArrCovId[id],fnalifeInsDets,fnaselfspsDets,strArrCovPlanName[id],incDate,expDate,strArrCovBasicRiderRef[id],strArrCovTypeofcov[id],dlbBoostr,dlbAmtAss,strArrCovExpPrd[id],strArrCovNoofyrs[id],dlbDsbenf,strArrCovYrsofpay[id],strArrCovMaxpayout[id],strArrCovTypesofHS[id],dlbDeduct,dlbCoIns,strArrCovDescription[id],strArrCovRemarks[id],strLoggedUser,new Date(),null,null));
							updliCovListAS.add(new FnaLifeinsuranceCoverages(strArrCovId[id],fnalifeInsDets,fnaselfspsDets,strArrCovTypeofcov[id],strArrliPlanPKId[id],strArrCovPlanName[id],strArrCovBasicRiderRef[id],incDate,expDate,null,null,null,null,dlbDeduct,dlbCoIns,strArrCovRemarks[id],strLoggedUser,new Date(),null,null,dlbBoostr,dlbAmtAss,strArrCovExpPrd[id],strArrCovNoofyrs[id],dlbDsbenf,strArrCovYrsofpay[id],strArrCovMaxpayout[id],strArrCovTypesofHS[id],strArrCovDescription[id]));
//							
...
//							updliCovList.add(new FnaLifeinsuranceCoverages(strArrCovId[id],fnalifeInsDets,fnaselfspsDets,strArrCovPlanName[id],incDate,expDate,strArrCovBasicRiderRef[id],strArrCovTypeofcov[id],dlbBoostr,dlbAmtAss,strArrCovExpPrd[id],strArrCovNoofyrs[id],dlbDsbenf,strArrCovYrsofpay[id],strArrCovMaxpayout[id],strArrCovTypesofHS[id],dlbDeduct,dlbCoIns,strArrCovDescription[id],strArrCovRemarks[id],strLoggedUser,new Date(),null,null));
							updliCovList.add(new FnaLifeinsuranceCoverages(strArrCovId[id],fnalifeInsDets,fnaselfspsDets,strArrCovTypeofcov[id],strArrliPlanPKId[id],strArrCovPlanName[id],strArrCovBasicRiderRef[id],incDate,expDate,null,null,null,null,dlbDeduct,dlbCoIns,strArrCovRemarks[id],strLoggedUser,new Date(),null,null,dlbBoostr,dlbAmtAss,strArrCovExpPrd[id],strArrCovNoofyrs[id],dlbDsbenf,strArrCovYrsofpay[id],strArrCovMaxpayout[id],strArrCovTypesofHS[id],strArrCovDescription[id]));
					...
					if(strArrCovMode[id].equalsIgnoreCase(FipaConstant.DELETE_MODE)){
						delliCovList.add(new FnaLifeinsuranceCoverages(strArrCovId[id],fnalifeInsDets,fnaselfspsDets,strArrCovPlanName[id],incDate,expDate,strArrCovBasicRiderRef[id],strArrCovTypeofcov[id],dlbBoostr,dlbAmtAss,strArrCovExpPrd[id],strArrCovNoofyrs[id],dlbDsbenf,strArrCovYrsofpay[id],strArrCovMaxpayout[id],strArrCovTypesofHS[id],dlbDeduct,dlbCoIns,strArrCovDescription[id],strArrCovRemarks[id],strLoggedUser,new Date(),null,null)); 
			...
			vectliCovDet.add(updliCovListAS);  
					...
					logger.debug("Error in getLiBenefitsDetails ->" , ex);
				}
	...
	public Vector getLiBenefitsDetails(HttpServletRequest request) {
		...
		if(strArrDsbMode != null && strArrDsbMode.length>0){
			...
			for(int id=0;id<strArrDsbMode.length;id++){
				...
				Double dlBenfVal=Double.parseDouble(FipaUtils.nullOrBlank(strArrDsbBenf[id]) ? "0" :strArrDsbBenf[id]);
				Double dlBenfIncr=Double.parseDouble(FipaUtils.nullOrBlank(strArrDsbIncBEnf[id]) ? "0" :strArrDsbIncBEnf[id]);
				 
				 
				 
				Date crtdDate = FipaUtils.nullOrBlank(strArrDsbCrtdDate[id]) ? new Date() :FipaDateUtils.convertStringToDate(strArrDsbCrtdDate[id]);
				
				Date incDate = FipaUtils.nullOrBlank(strArrDsbIncDate[id]) ? null :FipaDateUtils.convertStringToDate(strArrDsbIncDate[id]);
				Date expDate = FipaUtils.nullOrBlank(strArrDsbExpDate[id]) ? null :FipaDateUtils.convertStringToDate(strArrDsbExpDate[id]);
				...
				if(strArrDsbMode[id].equalsIgnoreCase(FipaConstant.INSERT_MODE) && FipaUtils.nullOrBlank(strPKId) ){
					insliBenefList.add(new FnaLifeinsuranceDisablebenfs(null,null,null,strArrDsbTypes[id],strArrDsbYrBegin[id],strArrDsbYrCeases[id],Double.parseDouble("0"),dlBenfVal,dlBenfIncr,strLoggedUser,new Date(),null,null,strArrDsbPlanName[id],incDate,expDate));
	 			
				 }
				
				if(strArrDsbMode[id].equalsIgnoreCase(FipaConstant.UPDATE_MODE)  || 
						(strArrDsbMode[id].equalsIgnoreCase(FipaConstant.INSERT_MODE) && !FipaUtils.nullOrBlank(strPKId))){
					if(strPKId.startsWith("AS")){
						updliBenefListAS.add(new FnaLifeinsuranceDisablebenfs(strArrDsbId[id],fnalifeInsDets,fnaselfspsDets,strArrDsbTypes[id],strArrDsbYrBegin[id],strArrDsbYrCeases[id],Double.parseDouble("0"),dlBenfVal,dlBenfIncr,strLoggedUser,new Date(),strLoggedUser,new Date(),strArrDsbPlanName[id],incDate,expDate)); 
						 
					}else{
						updliBenefList.add(new FnaLifeinsuranceDisablebenfs(strArrDsbId[id],fnalifeInsDets,fnaselfspsDets,strArrDsbTypes[id],strArrDsbYrBegin[id],strArrDsbYrCeases[id],Double.parseDouble("0"),dlBenfVal,dlBenfIncr,strArrDsbCrtdBy[id],crtdDate,strLoggedUser,new Date(),strArrDsbPlanName[id],incDate,expDate)); 
				...
				if(strArrDsbMode[id].equalsIgnoreCase(FipaConstant.DELETE_MODE)){
					delliBenefList.add(new FnaLifeinsuranceDisablebenfs(strArrDsbId[id],fnalifeInsDets,fnaselfspsDets,strArrDsbTypes[id],strArrDsbYrBegin[id],strArrDsbYrCeases[id],Double.parseDouble("0"),dlBenfVal,dlBenfIncr,strArrDsbCrtdBy[id],crtdDate,strLoggedUser,new Date(),strArrDsbPlanName[id],incDate,expDate)); 
		...
		vectliBenefDet.add(delliBenefList);vectliBenefDet.add(updliBenefListAS);  
		 }catch(Exception ex){
				logger.debug("Error in getLiBenefitsDetails ->" , ex);
			}
	...
	public Vector getLiNomNamesDetails(HttpServletRequest request) {
		...
		List updNomNameList = new ArrayList();	List updNomNameListAS = new ArrayList();
		...
		if(strArrNomNameMode != null && strArrNomNameMode.length>0){
			...
			for(int id=0;id<strArrNomNameMode.length;id++){
				...
				Date crtdDate = FipaUtils.nullOrBlank(strArrNomNameCrtdDate[id]) ? new Date() :FipaDateUtils.convertStringToDate(strArrNomNameCrtdDate[id]) ;
				double dlbnomper=Double.parseDouble(FipaUtils.nullOrBlank(strArrNomPer[id]) ? "0" :strArrNomPer[id]);
				//			
				FnaSelfspouseDets fnaselfspsDets = new FnaSelfspouseDets();				
				fnaselfspsDets.setFnaId(strFNAId);
				...
				if(strArrNomNameMode[id].equalsIgnoreCase(FipaConstant.INSERT_MODE) && FipaUtils.nullOrBlank(strPKId) ){
					insNomNameList.add(new FnaLifeinsuranceNominees(null,fnalifeInsDets,null,strArrNomName[id],strLoggedUser,new Date(),null,null,dlbnomper));
				 
				}
				
				if(strArrNomNameMode[id].equalsIgnoreCase(FipaConstant.UPDATE_MODE) || 
						(strArrNomNameMode[id].equalsIgnoreCase(FipaConstant.INSERT_MODE) && !FipaUtils.nullOrBlank(strPKId))){
					
					if(strPKId.startsWith("AS")){
						updNomNameListAS.add(new FnaLifeinsuranceNominees(strArrNomNameId[id],fnalifeInsDets,fnaselfspsDets,strArrNomName[id],strLoggedUser,new Date(),strLoggedUser,new Date(),dlbnomper));
						 
					}else{
						updNomNameList.add(new FnaLifeinsuranceNominees(strArrNomNameId[id],fnalifeInsDets,fnaselfspsDets,strArrNomName[id],strArrNomNameCrtdBy[id],crtdDate,strLoggedUser,new Date(),dlbnomper));
				...
				if(strArrNomNameMode[id].equalsIgnoreCase(FipaConstant.DELETE_MODE)){
					delNomNameList.add(new FnaLifeinsuranceNominees(strArrNomNameId[id],fnalifeInsDets,fnaselfspsDets,strArrNomName[id],strArrNomNameCrtdBy[id],crtdDate,strLoggedUser,new Date(),dlbnomper));
		...
		vectNomNameDetails.add(delNomNameList);vectNomNameDetails.add(updNomNameListAS);
		}catch(Exception ex){
			logger.debug("Error in getLiNomNamesDetails ->" , ex);
		}
	...
	public Vector getInputInvest(HttpServletRequest request){
		...
		List updInputInvestList = new ArrayList();List updInputInvestListAS = new ArrayList();
	...
	//	String[] strArrInvRegPlanToYrs		= FipaUtils.getParamArrValue(request, "txtFldInvRegPlanToYrs");//removed from screen
		...
		for(int id=0;id<strArrInvtmntMode.length;id++){
			...
			double DecInvEstrate=Double.parseDouble(FipaUtils.nullOrBlank(strArrInvEstrate[id]) ? "0" :strArrInvEstrate[id]);
			double DblInvAmount=Double.parseDouble(FipaUtils.nullOrBlank(strArrInvAmount[id]) ? "0" :strArrInvAmount[id]);
			double DblInvCurrbid=Double.parseDouble(FipaUtils.nullOrBlank(strArrInvCurrbid[id]) ? "0" :strArrInvCurrbid[id]);
			double DblInvUnitsAlloc=Double.parseDouble(FipaUtils.nullOrBlank(strArrInvUnitsAlloc[id]) ? "0" :strArrInvUnitsAlloc[id]);
		 
			double DblArrInvCurNav=Double.parseDouble(FipaUtils.nullOrBlank(strArrInvCurNav[id]) ? "0" :strArrInvCurNav[id]);
			 
			
			double regPlanAmout=Double.parseDouble(FipaUtils.nullOrBlank(strArrInvRegPlanAmount[id]) ? "0" :strArrInvRegPlanAmount[id]);
			
			double intInvRegPlanFromYrs=Double.parseDouble(FipaUtils.nullOrBlank(strArrInvRegPlanFromYrs[id]) ? "0" :strArrInvRegPlanFromYrs[id]);
	//		double intInvRegPlanToYrs=Double.parseDouble(FipaUtils.nullOrBlank(strArrInvRegPlanToYrs[id]) ? "0" :strArrInvRegPlanToYrs[id]);
			double intInvRegPlanToYrs=0;//removed from screen
			double intInvYrsToRet=Double.parseDouble(FipaUtils.nullOrBlank(strArrInvYrsToRet[id]) ? "0" :strArrInvYrsToRet[id]);
			double intInvAccPrcnt=Double.parseDouble(FipaUtils.nullOrBlank(strArrInvAccPrcnt[id]) ? "0" :strArrInvAccPrcnt[id]);
			
			double intArrInvAnnlDividPrct=Double.parseDouble(FipaUtils.nullOrBlank(strArrInvAnnlDividPrct[id]) ? "0" :strArrInvAnnlDividPrct[id]);
			
			double dblDividAmt=Double.parseDouble(FipaUtils.nullOrBlank(strArrDividendAmt[id]) ? "0" :strArrDividendAmt[id]);
			double dblDividPrd=Double.parseDouble(FipaUtils.nullOrBlank(strArrDivPeriod[id]) ? "0" :strArrDivPeriod[id]);
			double dblDividPARAmt=Double.parseDouble(FipaUtils.nullOrBlank(strArrDivPARAmt[id]) ? "0" :strArrDivPARAmt[id]);
			 
			
			double dblDisburseAmt=Double.parseDouble(FipaUtils.nullOrBlank(strArrDisburseAmt[id]) ? "0" :strArrDisburseAmt[id]);
			double dblDisburseYrs=Double.parseDouble(FipaUtils.nullOrBlank(strArrDisburseYrs[id]) ? "0" :strArrDisburseYrs[id]);
			
			
			Date DateInvDate = FipaUtils.nullOrBlank(strArrInvDate[id]) ? null :FipaDateUtils.convertStringToDate(strArrInvDate[id]);//Y NEW DATE IS ASSIGNED
			Date createdDate = FipaUtils.nullOrBlank(strArrInvCrtdDate[id]) ? new Date() :FipaDateUtils.convertStringToDate(strArrInvCrtdDate[id]);//Y NEW DATE IS ASSIGNED
			...
			if(strArrInvtmntMode[id].equalsIgnoreCase(FipaConstant.INSERT_MODE)  && FipaUtils.nullOrBlank(strPKId) ){
				insInputInvestList.add(new FnaInputinvestmentsDets(null,null,strArrInvOwner[id],strArrInvType[id],strArrInvFa[id],strArrInvAnlysByPtflio[id],strArrInvInstname[id],strArrInvDesc[id],DecInvEstrate,DblInvAmount,strArrInvPaymethod[id],DateInvDate,DblInvCurrbid,DblInvUnitsAlloc,DblArrInvCurNav,strArrInvAnyRegPlan[id],regPlanAmout,strArrInvRegPlanFreq[id],intInvRegPlanFromYrs,intInvRegPlanToYrs,strArrInvObjective[id],intInvYrsToRet,strArrInvChildname[id],intInvAccPrcnt,strArrInvRemarks[id],intArrInvAnnlDividPrct,strArrInvDividPaymode[id],strLoggedUser,new Date(),null,null,strArrInvRefId[id],dblDividAmt,strArrDividendBasdOn[id],strArrDividendReInv[id],dblDividPrd,dblDividPARAmt,strArrBasDisburse[id],dblDisburseAmt,dblDisburseYrs)); 
			 }
			
			if(strArrInvtmntMode[id].equalsIgnoreCase(FipaConstant.UPDATE_MODE)  || 
					(strArrInvtmntMode[id].equalsIgnoreCase(FipaConstant.INSERT_MODE) && !FipaUtils.nullOrBlank(strPKId))){	
				
				if(strPKId.startsWith("AS")){
					updInputInvestListAS.add(new FnaInputinvestmentsDets(strArrInvstId[id],fnaselfspsDets,strArrInvOwner[id],strArrInvType[id],strArrInvFa[id],strArrInvAnlysByPtflio[id],strArrInvInstname[id],strArrInvDesc[id],DecInvEstrate,DblInvAmount,strArrInvPaymethod[id],DateInvDate,DblInvCurrbid,DblInvUnitsAlloc,DblArrInvCurNav,strArrInvAnyRegPlan[id],regPlanAmout,strArrInvRegPlanFreq[id],intInvRegPlanFromYrs,intInvRegPlanToYrs,strArrInvObjective[id],intInvYrsToRet,strArrInvChildname[id],intInvAccPrcnt,strArrInvRemarks[id],intArrInvAnnlDividPrct,strArrInvDividPaymode[id],strLoggedUser,new Date(),strLoggedUser,new Date(),strArrInvRefId[id],dblDividAmt,strArrDividendBasdOn[id],strArrDividendReInv[id],dblDividPrd,dblDividPARAmt,strArrBasDisburse[id],dblDisburseAmt,dblDisburseYrs)); 
					
				}else{
					updInputInvestList.add(new FnaInputinvestmentsDets(strArrInvstId[id],fnaselfspsDets,strArrInvOwner[id],strArrInvType[id],strArrInvFa[id],strArrInvAnlysByPtflio[id],strArrInvInstname[id],strArrInvDesc[id],DecInvEstrate,DblInvAmount,strArrInvPaymethod[id],DateInvDate,DblInvCurrbid,DblInvUnitsAlloc,DblArrInvCurNav,strArrInvAnyRegPlan[id],regPlanAmout,strArrInvRegPlanFreq[id],intInvRegPlanFromYrs,intInvRegPlanToYrs,strArrInvObjective[id],intInvYrsToRet,strArrInvChildname[id],intInvAccPrcnt,strArrInvRemarks[id],intArrInvAnnlDividPrct,strArrInvDividPaymode[id],strArrInvCrtdBy[id],createdDate,strLoggedUser,new Date(),strArrInvRefId[id],dblDividAmt,strArrDividendBasdOn[id],strArrDividendReInv[id],dblDividPrd,dblDividPARAmt,strArrBasDisburse[id],dblDisburseAmt,dblDisburseYrs)); 
			...
			if(strArrInvtmntMode[id].equalsIgnoreCase(FipaConstant.DELETE_MODE)){
				delInputInvestList.add(new FnaInputinvestmentsDets(strArrInvstId[id],fnaselfspsDets,strArrInvOwner[id],strArrInvType[id],strArrInvFa[id],strArrInvAnlysByPtflio[id],strArrInvInstname[id],strArrInvDesc[id],DecInvEstrate,DblInvAmount,strArrInvPaymethod[id],DateInvDate,DblInvCurrbid,DblInvUnitsAlloc,DblArrInvCurNav,strArrInvAnyRegPlan[id],regPlanAmout,strArrInvRegPlanFreq[id],intInvRegPlanFromYrs,intInvRegPlanToYrs,strArrInvObjective[id],intInvYrsToRet,strArrInvChildname[id],intInvAccPrcnt,strArrInvRemarks[id],intArrInvAnnlDividPrct,strArrInvDividPaymode[id],strArrInvCrtdBy[id],createdDate,strLoggedUser,new Date(),strArrInvRefId[id],dblDividAmt,strArrDividendBasdOn[id],strArrDividendReInv[id],dblDividPrd,dblDividPARAmt,strArrBasDisburse[id],dblDisburseAmt,dblDisburseYrs));
			}
		...
		vectInputInvest.add(delInputInvestList);vectInputInvest.add(updInputInvestListAS);
		}catch(Exception ex){
			logger.debug("Error in getInputInvest ->" , ex);
		}
	...
	public Vector getOthRetPlg(HttpServletRequest request){
		...
		List updRdOthList = new ArrayList();	List updRdOthListAS = new ArrayList();
		...
		for(int id=0;id<strArrORMode.length;id++){
			...
			double dblAnlExp=Double.parseDouble(FipaUtils.nullOrBlank(strArrORAnlExp[id]) ? "0" :strArrORAnlExp[id]);
			double dblEscRte=Double.parseDouble(FipaUtils.nullOrBlank(strArrOREslrate[id]) ? "0" :strArrOREslrate[id]);
			 
			Date createdDate = FipaUtils.nullOrBlank(strArrORCrtdDate[id]) ? new Date() :FipaDateUtils.convertStringToDate(strArrORCrtdDate[id]);
			...
			if(strArrORMode[id].equalsIgnoreCase(FipaConstant.INSERT_MODE) && FipaUtils.nullOrBlank(strPKId) ){
				insRdOthList.add(new FnaRetireplanOthpayment(null,null,strArrORtyofpay[id],strArrORFreq[id],dblAnlExp,dblEscRte,strArrORAgeBsOn[id],strArrORAgePaySts[id],strArrORAgePayends[id],strArrORRemarks[id],strLoggedUser,new Date(),null,null));	 
			 }
			
			if(strArrORMode[id].equalsIgnoreCase(FipaConstant.UPDATE_MODE)|| 
					(strArrORMode[id].equalsIgnoreCase(FipaConstant.INSERT_MODE) && !FipaUtils.nullOrBlank(strPKId))){	
				
				if(strPKId.startsWith("AS")){
					updRdOthListAS.add(new FnaRetireplanOthpayment(strArrORId[id],fnaselfspsDets,strArrORtyofpay[id],strArrORFreq[id],dblAnlExp,dblEscRte,strArrORAgeBsOn[id],strArrORAgePaySts[id],strArrORAgePayends[id],strArrORRemarks[id],strLoggedUser,new Date(),strLoggedUser,new Date()));
				}else{
					updRdOthList.add(new FnaRetireplanOthpayment(strArrORId[id],fnaselfspsDets,strArrORtyofpay[id],strArrORFreq[id],dblAnlExp,dblEscRte,strArrORAgeBsOn[id],strArrORAgePaySts[id],strArrORAgePayends[id],strArrORRemarks[id],strArrORCrtdBy[id],createdDate,strLoggedUser,new Date()));
				}
			...
			if(strArrORMode[id].equalsIgnoreCase(FipaConstant.DELETE_MODE)){
				delRdOthList.add(new FnaRetireplanOthpayment(strArrORId[id],fnaselfspsDets,strArrORtyofpay[id],strArrORFreq[id],dblAnlExp,dblEscRte,strArrORAgeBsOn[id],strArrORAgePaySts[id],strArrORAgePayends[id],strArrORRemarks[id],strArrORCrtdBy[id],createdDate,strLoggedUser,new Date())); 
			}
		...
		vectRdOth.add(delRdOthList);vectRdOth.add(updRdOthListAS);
		}
		catch(Exception ex){
			logger.debug("Error in getOthRetPlg ->" , ex);
		}
	...
	public Vector getIncRetPlg(HttpServletRequest request) {
		...
		List updRdInAssList = new ArrayList();List updRdInAssListAS = new ArrayList();
		...
		for(int id=0;id<strArrincretMode.length;id++){
			...
			double dblIncAmt = Double.parseDouble(FipaUtils.nullOrBlank(strArrIRAmtofInc[id]) ? "0" :strArrIRAmtofInc[id]);
			double dblIncEsc = Double.parseDouble(FipaUtils.nullOrBlank(strArrIREslrate[id]) ? "0" :strArrIREslrate[id]);
			double dblIncRoi = Double.parseDouble(FipaUtils.nullOrBlank(strArrIRRoi[id]) ? "0" :strArrIRRoi[id]);
			
			Date crtdDate = FipaUtils.nullOrBlank(strArrIRCrtdDate[id]) ? new Date() :FipaDateUtils.convertStringToDate(strArrIRCrtdDate[id]) ;
			...
			if(strArrincretMode[id].equalsIgnoreCase(FipaConstant.INSERT_MODE) && FipaUtils.nullOrBlank(strPKId)){
				insRdInAssList.add(new FnaRetireplanIncomeasset(null,null,"RETINCOME",strArrIRClsfy[id],strArrIRDesc[id],strArrIRFreq[id],dblIncAmt,dblIncEsc,dblIncRoi,strArrIRAgeBasOn[id],strArrIRAgePaySts[id],strArrIRAgePayends[id],strLoggedUser,new Date(),null,null,""));
	 
			}
			
			if(strArrincretMode[id].equalsIgnoreCase(FipaConstant.UPDATE_MODE)|| 
					(strArrincretMode[id].equalsIgnoreCase(FipaConstant.INSERT_MODE) && !FipaUtils.nullOrBlank(strPKId))){
				
				if(strPKId.startsWith("AS")){
					updRdInAssListAS.add(new FnaRetireplanIncomeasset(strArrIncId[id],fnaselfspsDets,"RETINCOME",strArrIRClsfy[id],strArrIRDesc[id],strArrIRFreq[id],dblIncAmt,dblIncEsc,dblIncRoi,strArrIRAgeBasOn[id],strArrIRAgePaySts[id],strArrIRAgePayends[id],strLoggedUser,new Date(),strLoggedUser,new Date(),""));
					 
				}else{
					updRdInAssList.add(new FnaRetireplanIncomeasset(strArrIncId[id],fnaselfspsDets,"RETINCOME",strArrIRClsfy[id],strArrIRDesc[id],strArrIRFreq[id],dblIncAmt,dblIncEsc,dblIncRoi,strArrIRAgeBasOn[id],strArrIRAgePaySts[id],strArrIRAgePayends[id],strArrIRCrtdBy[id],crtdDate,strLoggedUser,new Date(),""));
			...
			if(strArrincretMode[id].equalsIgnoreCase(FipaConstant.DELETE_MODE)){
				delRdInAssList.add(new FnaRetireplanIncomeasset(strArrIncId[id],fnaselfspsDets,"RETINCOME",strArrIRClsfy[id],strArrIRDesc[id],strArrIRFreq[id],dblIncAmt,dblIncEsc,dblIncRoi,strArrIRAgeBasOn[id],strArrIRAgePaySts[id],strArrIRAgePayends[id],strArrIRCrtdBy[id],crtdDate,strLoggedUser,new Date(),""));
		...
		vectRdInAssDetails.add(delRdInAssList);vectRdInAssDetails.add(updRdInAssListAS);
		
		 
		if(strArrincassrtMode != null && strArrincassrtMode.length>0){
			for(int id=0;id<strArrincassrtMode.length;id++){
				double dblIncAssAmt = Double.parseDouble(FipaUtils.nullOrBlank(strArrIncAssAmt[id]) ? "0" :strArrIncAssAmt[id]);
				double dblIncAssEsc = Double.parseDouble(FipaUtils.nullOrBlank(strArrIncAssEsc[id]) ? "0" :strArrIncAssEsc[id]);
				double dblIncAssRoi = Double.parseDouble(FipaUtils.nullOrBlank(strArrIncAssRoi[id]) ? "0" :strArrIncAssRoi[id]);
				
				Date crtdDate = FipaUtils.nullOrBlank(strArrIncAssCrtdDate[id]) ? new Date() :FipaDateUtils.convertStringToDate(strArrIncAssCrtdDate[id]) ;
				...
				if(strArrincassrtMode[id].equalsIgnoreCase(FipaConstant.INSERT_MODE) && FipaUtils.nullOrBlank(strPKId) ){
					insRdInAssList.add(new FnaRetireplanIncomeasset(null,null,"RETINCOMEASS",strArrIncAssClsfy[id],strArrIncAssDesc[id],strArrIncAssFreq[id],dblIncAssAmt,dblIncAssEsc,dblIncAssRoi,strArrIncAssAgeBsOn[id],strArrIncAssAgePaysts[id],strArrIncAssAgePayends[id],strLoggedUser,new Date(),null,null,strArrIncAssRefId[id]));
		 
				}
				
				if(strArrincassrtMode[id].equalsIgnoreCase(FipaConstant.UPDATE_MODE) || 
						(strArrincassrtMode[id].equalsIgnoreCase(FipaConstant.INSERT_MODE) && !FipaUtils.nullOrBlank(strPKId))){
					
					if(strPKId.startsWith("AS")){
						updRdInAssListAS.add(new FnaRetireplanIncomeasset(strArrIncAssId[id],fnaselfspsDets,"RETINCOMEASS",strArrIncAssClsfy[id],strArrIncAssDesc[id],strArrIncAssFreq[id],dblIncAssAmt,dblIncAssEsc,dblIncAssRoi,strArrIncAssAgeBsOn[id],strArrIncAssAgePaysts[id],strArrIncAssAgePayends[id],strLoggedUser,new Date(),strLoggedUser,new Date(),strArrIncAssRefId[id]));
						 
					}else{
						updRdInAssList.add(new FnaRetireplanIncomeasset(strArrIncAssId[id],fnaselfspsDets,"RETINCOMEASS",strArrIncAssClsfy[id],strArrIncAssDesc[id],strArrIncAssFreq[id],dblIncAssAmt,dblIncAssEsc,dblIncAssRoi,strArrIncAssAgeBsOn[id],strArrIncAssAgePaysts[id],strArrIncAssAgePayends[id],strArrIncAssCrtdBy[id],crtdDate,strLoggedUser,new Date(),strArrIncAssRefId[id]));
				...
				if(strArrincassrtMode[id].equalsIgnoreCase(FipaConstant.DELETE_MODE)){
					delRdInAssList.add(new FnaRetireplanIncomeasset(strArrIncAssId[id],fnaselfspsDets,"RETINCOMEASS",strArrIncAssClsfy[id],strArrIncAssDesc[id],strArrIncAssFreq[id],dblIncAssAmt,dblIncAssEsc,dblIncAssRoi,strArrIncAssAgeBsOn[id],strArrIncAssAgePaysts[id],strArrIncAssAgePayends[id],strArrIncAssCrtdBy[id],crtdDate,strLoggedUser,new Date(),strArrIncAssRefId[id]));
		...
		vectRdInAssDetails.add(delRdInAssList);vectRdInAssDetails.add(updRdInAssListAS);
			...
			logger.debug("Error in getIncRetPlg ->" , ex);
		}
	...
	private Vector uploadAttachments(HttpServletRequest request,
			CommonsMultipartFile[] fipaAttachmentDoc)  {
		// TODO Auto-generated method stub
	  ...
	  if (fipaAttachmentDoc != null && fipaAttachmentDoc.length > 0) {
		...
		for (CommonsMultipartFile file : fipaAttachmentDoc){ 
			...
			if(strArrAttchMode[id].equalsIgnoreCase(FipaConstant.INSERT_MODE)){
				...
				strBFILEName = File.separator+File.separator +strAdvIdDir.trim() +File.separator+File.separator +  strCustName.trim() +"_"+ strCustNRIC.trim() + File.separator +File.separator+strAtmtFileName +strBFILEName;	    					
	    	  ...
	    	  if (!fipaAttachmentDoc[id].getOriginalFilename().equals("")) {
	 		    	File srcFile  = FipaUtils.createPhysicalDir(FipaConstant.GLBL_MODULE_ATTACTMENT,strLoggedAdvName,strCustName,strCustNRIC,strAtmtFileName);
	 		    			
	 		    	retMsg = FipaUtils.createPhysicalFile(new File(fipaAttachmentDoc[id].getOriginalFilename()), srcFile);
	    	  ...
	    	  if(!fipaAttachmentDoc[id].isEmpty()){		
	 			 StringBuffer ATTCH_INSERT_QRY = new StringBuffer(" INSERT INTO "+FipaConstant.FIPA_ATTACH_DB_TBL+" ("
	 					...
	 					+ " VALUES ('").append(maxAttachId)
							...
							.append(" ,BFILENAME (").append("'"+orcleDir+"','"+strBFILEName+"')")
							.append(" ,'").append(strArrCatgId[id]).append("'")
							.append(" ,'").append(strArrCatgName[id]).append("'")
							.append(" ,'").append(strArrDocTitle[id]).append("'")
							.append(" ,'").append(strArrAttPageNo[id]).append("'")
							.append(" ,'").append(strAtmtFileName).append("'")
							.append(" ,'").append(strFileSize).append("'")
							.append(" ,'").append(strFileContentType).append("'")
							.append(" ,'").append(strLoggedUser).append("'")
							.append(" ,SYSDATE")
							.append(" ,'").append(strArrAttRemarks[id]).append("'") 
							.append(" ,SYSDATE")
							.append(" ,'").append("FIPA_ATTACH").append("'")
			...
			if(strArrAttchMode[id].equalsIgnoreCase(FipaConstant.QUERY_MODE)){ 
				  if(!strArrDocid[id].isEmpty()){	
						 String ATTCH_UPDATE_QRY = " INSERT INTO "+FipaConstant.FIPA_ATTACH_DB_TBL+" ("
				 					+ "DOCID,FNA_ID,DOCUMENT,ATTACH_CATEG_ID,ATTACH_CATEG_NAME,DOCUMENT_TITLE,PAGE_NO,FILENAME,FILESIZE,FILETYPE,"
				 					+ "CREATED_BY,ATTACH_DATE,REMARKS,CREATED_DATE,MODULE_REF) "
				 					+ " select\n" + 
				 					"				  "+maxAttachId+",\n" + 
				 					"				  '<FNAID>',\n" + 
				 					"				  DOCUMENT,\n" + 
				 					...
				 					"				  SYSDATE,\n" + 
				 					"				  MODULE_REF\n" + 
				 					"				  from "+FipaConstant.FIPA_ATTACH_DB_TBL+" WHERE DOCID='"+strArrDocid[id]+"'";
