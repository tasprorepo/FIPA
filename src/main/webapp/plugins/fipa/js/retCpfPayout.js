...
function defaultCpflifevalues(){
	console.log("======defaultCpflifevalues")
...
$lastRow.find("td:eq(3)").find('select:eq(0)').on("change",function(){
	ajaxCalRetirementCpflife($(this),$lastRow.find("td:eq(4)").find('input:eq(0)').val(),$lastRow.find("td:eq(5)").find('input:eq(0)'),$lastRow.find("td:eq(6)").find('input:eq(0)'));	
//	cpflifefilldlgval($lastRow); 
//	instantRetCpfPaySave($lastRow,UPD_MODE);	
//	applyToastrAlert(FDTRNCPFLFE_INS_SAVE);
});
...
if(dataset == null){
	instantRetCpfPaySave($lastRow,INS_MODE);		
} */
...
function ajaxCalRetirementCpflife(cpftype,cpfprem,cpfpayout,fdtransfer){
	...
	console.log("======ajaxCalRetirementCpflife")
 if(cpftype == null)
	 cpftype ="";
	...
	var cpfType= ( cpftype == null || cpftype =='null' || isEmpty(cpftype) )? "RP" : cpftype;//plan code  
