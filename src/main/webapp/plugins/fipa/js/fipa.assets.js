...
//		sumspsloan+=loanOtherAssts;
	 ...
	 if(!( sumslfloan==0 && sumspsloan==0 )){
		 $("#liabSelfOthers").val(remPriceAfDec(sumslfloan));
		 $("#liabSpsOthers").val(remPriceAfDec(sumspsloan));
		 $("#liabFamOthers").val(remPriceAfDec(sumjointloan));
...
function valperastTbl(){   
	...
	if($RowCount > 0 ){ 
		...
		if(!(validateFocusDhtmlFlds($lastRow.find("td:eq(2)").find('select:eq(0)'), PERACCHOL))){valid=false;return false;}; 
		if(!(validateFocusDhtmlFlds($lastRow.find("td:eq(3)").find('input:eq(0)'), PERACCTP))){valid=false;return false;}; 
		if(!(validateFocusDhtmlFlds($lastRow.find("td:eq(4)").find('input:eq(0)'), PERASTNAME))){valid=false;return false;};  
