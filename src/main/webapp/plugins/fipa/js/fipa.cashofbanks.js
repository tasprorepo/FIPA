...
var cell2 = '<select name="txtFldMainAccHolderName" class="form-control editable"  onmouseover="fipaTooltip(this);"   maxlength="75"></select>'; 
...
$lastRow.find("td:eq(11)").find('select:eq(0)').on("change",function(){
//	reverseCobSync($lastRow);	
}); 
...
$lastRow.find("td:eq(15)").find('input:eq(0)').on("change",function(evt){ 
...
$lastRow.find("td:eq(17)").find('input:eq(3)').val($("#txtFldDlgCOBYrsToRet").val());
...
$lastRow.find("input,select").on("click",function(event){
	...
	if(dataset != null){
		...
		var owner = $lastRow.find("td:eq(5)").find('select:eq(0)').val();
		var objective = $lastRow.find("td:eq(14)").find('select:eq(0)').val() ;
...
//				 $curElm.parents("tr").find("td:first").find('input:eq(0)').val($mode);
				 ...
				 $(this).parents("tr").find("td").find("input.editable,select.editable").each(function(){
					...
					openDivCobObject($row.find("td:eq(14)").find('select:eq(0)'),$row.find("td:eq(15)").find('input:eq(0)'),$row.find("td:eq(16)").find('select:eq(0)'));
					showFIPAModel('Cob_Dialog','Cash At Banks');  
...
//	  $('#Cob_Dialog #txtFldDlgCashBankMode').val($lastRow.find("td:eq(0)").find('input:eq(0)').val());//instant save added line
	  ...
	  $('#Cob_Dialog #txtFldDlgCOBYrsToRet').val($lastRow.find("td:eq(17)").find('input:eq(3)').val());
...
function Cobfilldomval($RowId,$row){
	...
	$row.find("td:eq(17)").find('input:eq(3)').val($("#txtFldDlgCOBYrsToRet").val());
...
$("#Cob_Dialog").find("input,select,textarea").on("change",function(){
	$("#Cob_Dialog").find(".modal-footer").find("button:eq(0)").removeClass("hidden");
});
...
function syncCashAtTblRow($tblCabRow){
	...
	var owner = $tblCabRow.find("td:eq(5)").find('select:eq(0)').val();
	var objective = $tblCabRow.find("td:eq(14)").find('select:eq(0)').val() ;
		...
		if(!(owner == "Spouse" )){
			...
			//}
				...
				var totAge=isEmpty(projselfage)? 0 :Number(projselfage);
				 ...
				 var bankname = $tblCabRow.find("td:eq(6) input:eq(0)").val();
				 var acctno= $tblCabRow.find("td:eq(7) input:eq(0)").val();
				 var currentbalance = $tblCabRow.find("td:eq(9) input:eq(0)").val();
				 
				 var rspamt = $tblCabRow.find("td:eq(10) input:eq(0)").val();
				 var rspfreq = $tblCabRow.find("td:eq(11) select:eq(0)").val();
				 var rspfromdate = $tblCabRow.find("td:eq(12) input:eq(0)").val();
				 var rsptodate = $tblCabRow.find("td:eq(13) input:eq(0)").val();
				 
				 rspamt = isEmpty(rspamt)?0:Number(rspamt);
				 
				 var invobj = $tblCabRow.find("td:eq(14) select:eq(0)").val(); 
				 var retirementprcnt = $tblCabRow.find("td:eq(15) input:eq(0)").val();
				var yrstoret = $tblCabRow.find("td:eq(17)").find('input:eq(3)').val();
...
//						 pvannlincome = Math.sign(pvannlincome) == -1 ? pvannlincome : (-1 *pvannlincome);
					...
					$tblRetRow.find("td:eq(5)").find('input:eq(0)').val(Math.round(amountofincome)); 
...
//					$tblRetRow.find("td:eq(10)").find('input:eq(0)').val(rspfreq == "REGULAR" ? ((isEmpty(totAge)) ? Number("0") : Number(totAge)) : "");
					$tblRetRow.find("td:eq(10)").find('input:eq(0)').val("").prop("readonly",true);
...
function setCashAtBankToAsset(latestrow){
	...
	if(length>0){
		...
		$('#cashOfBanksTable tbody tr').each(function(){
			...
			var owner = $lastRow.find("td:eq(5)").find('select:eq(0)').val();
			var accttype = $lastRow.find("td:eq(8)").find('select:eq(0)').val();
			var currbalance = $lastRow.find("td:eq(9)").find('input:eq(0)').val();
			var objective = $lastRow.find("td:eq(14)").find('select:eq(0)').val() ;
			...
			var prcnt = $lastRow.find("td:eq(15)").find('input:eq(0)').val() ;
			prcnt = isEmpty(prcnt) ? 0 : Number(prcnt / 100);
			
			var latestprcnt = latestrow.find("td:eq(15)").find('input:eq(0)').val();
			var lastprcnt = $lastRow.find("td:eq(15)").find('input:eq(0)').val();
			var latestowner = latestrow.find("td:eq(5)").find('select:eq(0)').val();
