...
//	var $lastRow = $("#deptParticularsTable tbody tr:last");	
	...
	if($RowCount > 0 ){ 
		$("#deptParticularsTable tbody tr").each(function(){
			var $lastRow=$(this);
		if(!(validateFocusDhtmlFlds($lastRow.find("td:eq(2)").find('input:eq(0)'), DEPN_NAME))){valid=false;return false;}; 
		if(!(validateFocusDhtmlFlds($lastRow.find("td:eq(3)").find('select:eq(0)'), DEPN_RELATION))){valid=false;return false;};
		if(!(validateFocusDhtmlFlds($lastRow.find("td:eq(4)").find('input:eq(0)'), DEPN_DOB))){valid=false;return false;};
...
function deptfilldomval($RowId,$row){
	...
	$row.find("td:eq(4)").find('select:eq(0)').val($("#txtFldDlgDepnDob").val()); 
	$row.find("td:eq(5)").find('select:eq(0)').val($("#txtFldDlgDepnAge").val());  
...
//	 if(!(sum1 == 0 && sum2 == 0 && sum3 == 0)){ 
		 $("#deptParticularsTablefooter #txtFldDepdTotMthContr").val(remPriceAfDec(sum1)); 
		 $("#deptParticularsTablefooter #txtFldDepdTotMASelf").val(remPriceAfDec(sum2));
		 $("#deptParticularsTablefooter #txtFldDepdTotAnnlSelf").val(sum2*12);
		 $('#expdSelfDepntcontr').val(sum2*12); 
		 $("#deptParticularsTablefooter #txtFldDepdTotMASps").val(remPriceAfDec(sum3));
...
//		if(!(sumMthContb == 0)){
			chgeto.val(remPriceAfDec(Number(selfval)+Number(spouseval))); 
//		}else{
//			chgeto.val("");
//		}
