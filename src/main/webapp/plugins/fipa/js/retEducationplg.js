...
function calPVFVRetEduPlg($lastRow){
	...
	$lastRow.find("td:eq(13)").find('input:eq(0)').val(remPriceAfDec(totTeEdAge));
	totalPvRetEdPlgTerEdAge();
}
...
function calculateTerEduAgeReachedOut(){
	...
	var terAge=remPriceAfDec(Number($("#txtFldDlgEdPlgTerEdcAge").val()));
...
function totalPvTerEdAge(){ 
	...
	$("#pvEdAgeCalTablefooter #txtFldTotalPVCal").val(remPriceAfDec(sum));
}
