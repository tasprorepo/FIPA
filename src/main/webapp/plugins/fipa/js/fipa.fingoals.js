...
//	var $lastRow = $("#finGoalsTable tbody tr:last");	
	...
	if($RowCount > 0 ){ 
		$("#finGoalsTable tbody tr").each(function(){
			var $lastRow=$(this);
		if(!(validateFocusDhtmlFlds($lastRow.find("td:eq(2)").find('select:eq(0)'), FINTYPE_GOALS))){valid=false;return false;};
		if(!(validateFocusDhtmlFlds($lastRow.find("td:eq(3)").find('input:eq(0)'), FIN_GOALS))){valid=false;return false;}; 
