...
//	showPopOver('backToFdFlowInv', STR_BACKTOFDFLW);
//	showPopOver('backToFdFlowLife', STR_BACKTOFDFLW);
...
function fipaSearch() {
	...
	if (stafftype == STAFFTYPE_ADVISER) {
		$.each(srchFldsArrExclAll, function(index, value) {
			if (!isEmpty($("#" + srchFldsArrExclAll[index]).val().trim())) {
		...
		if (!isAnyAdvSelected) {
			applyToastrAlert("Select any search criteria"); 
			clearDataTable('ClientSearchTable');
			return false;
		}

	} else if (stafftype == STAFFTYPE_STAFF) {
		if (!isEmpty($("#txtFldSrchCustNric").val().trim())) {
			isAnyStfSelected = true;

		}

		if (!isAnyStfSelected) { 
			applyToastrAlert(NRIC_KEYIN, txtFldSrchCustNric); 
			clearDataTable('ClientSearchTable');
			return false;
		}
	}
...
////		fipaForm.submit();
//
//	} else if (!isEmpty(strFNAId)) { 
////		if(valSelfSpsRemarks()){
////			console.log("UpdateProfile.do")
////		}
//		valSelfSpsRemarks("UPDATE");
//		
...
//								profileTbl.clear().draw();
								...
								for ( var cont in jsnData) {
									...
									if (jsnData.hasOwnProperty(cont)) {
										...
										var cell2 = crow.insertCell(2);
										cell2.innerHTML = '<input type="text" readOnly="true"  class="dhtmlTableText ptrcursor"  style="text-align:left;"/>'
...
//										cell6.innerHTML = contvalue["strFnaAnaReplace"];
										...
										cell7.innerHTML = "<div><textarea class='form-control' disabled='true' rows='3' readOnly='true' onmouseover='fipaTooltip(this);'  >"+contvalue["strFnaAdvRemarks"]+"</textarea></div>";
...
//										var emptycell = "";
										...
										$lastRow.click(function() {callFipaInsuranceDets($(this));});
...
//										 		 }
										 ...
										 if(paymtd ==  "CASH" || paymtd ==  "CHEQUE" ||	 paymtd ==  "CREDIT-CARD" || paymtd ==  "GIRO - POSB" ||
												 paymtd ==  "TT" || paymtd ==  "GIRO - OTHERS" || paymtd ==  "CASH CARD" || paymtd ==  "CASH/CHQ" || paymtd == "CASH/CHEQUE"){
									 ...
									 $("#expdSelfInsurance").val(remPriceAfDec(strPremAmtSelf)); 
								     ...
								     $("#expdSpsInsurance").val(remPriceAfDec(strPremAmtSpouse));
								     ...
								     $("#expdFamilyInsurance").val(remPriceAfDec(strPremAmtFamily));   
