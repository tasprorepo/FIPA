...
//	loadAgeStartEnd($lastRow.find("td:eq(7)").find('select:eq(0)'),$(this)); 
...
//var sel1 = $("#txtFldDlgORAgePaySts > option").clone();
//$lastRow.find("td:eq(7)").find('input:eq(0)').append(sel1);
...
//	loadAgeEnd($(this),$lastRow.find("td:eq(8)").find('select:eq(0)'));
	if(!rdStartAgeValidate($(this),$lastRow.find("td:eq(6)").find('select:eq(0)')))return;
});

//var sel2 = $("#txtFldDlgORAgePayends > option").clone();
//$lastRow.find("td:eq(8)").find('input:eq(0)').append(sel2);
	...
	for(var data in dataset){
		...
		switch(data){
		 ...
		 case "opId": 
			...
			case "retAgebasedon":   
//				loadAgeStartEnd($lastRow.find("td:eq(7)").find('select:eq(0)'),$lastRow.find("td:eq(6)").find('select:eq(0)'));
				selectNullvalChk($lastRow.find("td:eq(6)"),col);  
//				loadAgeEnd($lastRow.find("td:eq(7)").find('select:eq(0)'),$lastRow.find("td:eq(8)").find('select:eq(0)'));
				break;
...
//				loadAgeEnd($lastRow.find("td:eq(7)").find('select:eq(0)'),$lastRow.find("td:eq(8)").find('select:eq(0)'));
//				selectNullvalChk($lastRow.find("td:eq(8)"),dataset["retAgeend"]);    
				break;
...
//	var $lastRow = $("#OthRetPlgtbl tbody tr:last");	
	...
	if($RowCount > 0 ){
		$("#OthRetPlgtbl tbody tr").each(function(){
			var $lastRow=$(this);
		if(!(validateFocusDhtmlFlds($lastRow.find("td:eq(2)").find('input:eq(0)'), OR_TYPE))) {valid=false;return false;};
		if(!(validateFocusDhtmlFlds($lastRow.find("td:eq(3)").find('select:eq(0)'), OR_FREQ))) {valid=false;return false;};  
		if(!(validateFocusDhtmlFlds($lastRow.find("td:eq(6)").find('select:eq(0)'), OR_AGEBSE))){valid=false;return false;};
		if(!(validateFocusDhtmlFlds($lastRow.find("td:eq(7)").find('input:eq(0)'), OR_AGESTS))){valid=false;return false;};
...
//	var $lastRow = $("#IncRetPlgtbl tbody tr:last");	
	...
	if($RowCount > 0 ){ 
		$("#IncRetPlgtbl tbody tr").each(function(){
			var $lastRow=$(this);
		if(!(validateFocusDhtmlFlds($lastRow.find("td:eq(2)").find('input:eq(0)'), IR_CLSFY))){valid=false;return false;};
		if(!(validateFocusDhtmlFlds($lastRow.find("td:eq(4)").find('select:eq(0)'), IR_FRQ))) {valid=false;return false;}; 
		if(!(validateFocusDhtmlFlds($lastRow.find("td:eq(8)").find('select:eq(0)'), IR_AGEBASED))){valid=false;return false;};
		if(!(validateFocusDhtmlFlds($lastRow.find("td:eq(9)").find('input:eq(0)'), IR_AGESTS))){valid=false;return false;};
...
var cell2 = '<input type="text" name="txtFldIncAssClsfy" class="form-control editable"   onmouseover="fipaTooltip(this);" maxlength="150" /><input type="hidden" name="txtFldIncAsstSubClsfy"/>'; 
...
$lastRow.find("td:eq(2)").find('input:eq(0)').val(classification);
$lastRow.find("td:eq(3)").find('input:eq(0)').val(desc);
...
$lastRow.find("td:eq(5)").find('input:eq(0)').val(amountofincome);
$lastRow.find("td:eq(5)").find('input:eq(0)').addClass("applyEvntUsd");
...
$lastRow.find("td:eq(6)").find('input:eq(0)').val(esclatrate);
$lastRow.find("td:eq(6)").find('input:eq(0)').addClass("applyEvntpCent3");

$lastRow.find("td:eq(7)").find('input:eq(0)').val(roi); 
$lastRow.find("td:eq(7)").find('input:eq(0)').addClass("applyEvntpCent3");

var iassagebsed =  $("#selDlgIncAssAgeBsOn > option").clone();
$lastRow.find("td:eq(8)").find('select:eq(0)').append(iassagebsed);
$lastRow.find("td:eq(8)").find('select:eq(0)').val(agebased);
$lastRow.find("td:eq(8)").find('select:eq(0)').on("change",function(){ 
	if(!rdStartAgeValidate($lastRow.find("td:eq(9)").find('input:eq(0)'),$(this)))return;
//	reverseRetSync($lastRow);//Reverse Sync process
}); 
...
$lastRow.find("td:eq(9)").find('input:eq(0)').val(agestarts);
$lastRow.find("td:eq(9)").find('input:eq(0)').addClass("applyEvntYrs");
$lastRow.find("td:eq(9)").find('input:eq(0)').on("change",function(){
	if(!rdStartAgeValidate($(this),$lastRow.find("td:eq(8)").find('select:eq(0)')))return;
}); 

$lastRow.find("td:eq(10)").find('input:eq(0)').val(ageends);
$lastRow.find("td:eq(10)").find('input:eq(0)').addClass("applyEvntYrs");
$lastRow.find("td:eq(10)").find('input:eq(0)').on("change",function(){
...
//	var $lastRow = $("#IncAssRetPlgtbl tbody tr:last");	
	...
	if($RowCount > 0 ){ 
		$("#IncAssRetPlgtbl tbody tr").each(function(){
			var $lastRow=$(this);
		if(!(validateFocusDhtmlFlds($lastRow.find("td:eq(2)").find('input:eq(0)'), INCASS_CLSFY))) {valid=false;return false;}; 
		if(!(validateFocusDhtmlFlds($lastRow.find("td:eq(4)").find('select:eq(0)'), INCASS_FREQ))) {valid=false;return false;};
		if(!(validateFocusDhtmlFlds($lastRow.find("td:eq(8)").find('select:eq(0)'), INCASS_AGEBSD))){valid=false;return false;};
		if(!(validateFocusDhtmlFlds($lastRow.find("td:eq(9)").find('input:eq(0)'), INCASS_AGESTS))){valid=false;return false;};
