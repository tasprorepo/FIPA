...
//										arrHdr=(test)?(hdrTest+"-"+count):hdr;
											...
											if($agebasedlt == "SELF"){ 
												if($freqlt == "REGULAR"){
													if(i >= $startslfagelt &&  (i<=$endagelt)){ 
														if(i==$startslfagelt){ 
															arrlist[arrHdr]=(isNaN($anlexplt)) ? 0 : RemDecimalNumber(Number($anlexplt));  
														}else{
															var olddata=dataset[dataset.length-1][arrHdr]; 
															olddata*=(1+($esclratelt/100));
															arrlist[arrHdr]=(isNaN(olddata)) ? 0 : RemDecimalNumber(Number(olddata));  
														}													
													}else{
														arrlist[arrHdr]=''; 
													}
						...
						var annalexp =  isNaN(totAnlExp) ? 0 : (headlist.length > 0 ? RemDecimalNumber( Number(adj) + Number(totAnlExp) ): RemDecimalNumber(Number(totAnlExp))); 
						...
						if(i==intretslfage){ 
							...
							arrlist["Total Accumulation Expenditure"]= annalexp; //(isNaN(totAnlExp)) ? 0 : RemDecimalNumber(Number(adj)+Number(totAnlExp));  
							oldval= annalexp;//Number(adj)+totAnlExp;
							accmlexp = oldval;
						}else{
							console.log("1.second itetation----------->");
							arrlist["Total Accumulation Expenditure"]=(isNaN(annalexp + oldval)) ? 0 : RemDecimalNumber( Number(annalexp) + Number(oldval) );
