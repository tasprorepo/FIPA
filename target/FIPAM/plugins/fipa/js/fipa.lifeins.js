...
//	 if(!validationRetirementScreen())return;  
	 enableRetCashValOnRet($("#retMultionret"),$("#retCashvalonret"),true,$("#lipRetRefId").val(),$("#covRef").text()); 
//	 $("#lipRetRefId").val("");
...
//							var ref = contvalue["strFPMSPolPrdtType"]; 
//									if(contvalue["strFPMSPolPrdtType"] == "B"){
//										getliPlndetRows(contvalue, tab);
//									}else if(contvalue["strFPMSPolPrdtType"] == "R"){
//										getliBenfRaidDetRows(contvalue, tab);  
//									} 
						});
						
//						 var count  = liRaiderDetailstbl.rows().count();
//						 if(count>0){
//							$("#liRaiderDetailstbl tbody tr").each(function(){ 
//							 $(this).find("td:eq(0)").find('input:eq(3)').addClass("planrefid").val("R_0");
//							});
//						 }
					}
				}
...
//		});
	...
	//			 } 
				 ...
				 $tblCpfRow.find("td:eq(9)").find('input:eq(0)').val(remPriceAfDec(amount*frequency));//Amount 
