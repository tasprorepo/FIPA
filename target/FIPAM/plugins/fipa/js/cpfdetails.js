...
function valadctTbl(){  
	...
	if($RowCount > 0 ){ 
		...
		if(!(validateFocusDhtmlFlds($lastRow.find("td:eq(2)").find('select:eq(0)'), CAD_APPLICANT))){valid=false;return false;}; 
		if(!(validateFocusDhtmlFlds($lastRow.find("td:eq(5)").find('select:eq(0)'), CAD_TRANSTYPE))){valid=false;return false;};
		if(!(validateFocusDhtmlFlds($lastRow.find("td:eq(6)").find('select:eq(0)'), CAD_ACCTYPE))){valid=false;return false;};
...
function calcCPFContribution(){
	...
	if(!isEmpty(selfdob) && !isEmpty(slfow)){
		...
		$("input[name=ccEmpleContrbSelf]").val(isNaN(Number(remPriceAfDec(ee * selftotcontrib)))?"0":Number(remPriceAfDec(ee * selftotcontrib)));
		$("input[name=ccEmplrContrbSelf]").val(isNaN(Number(remPriceAfDec(er * selftotcontrib)))?"0":Number(remPriceAfDec(er * selftotcontrib))); 
...
//	if(!isValidObject(er)){er="";}
	...
	if(!isEmpty(spsdob) && !isEmpty(spsow)){
		...
		$("input[name=ccEmpleContrbSps]").val(isNaN(Number(remPriceAfDec(ee1 * spsftotcontrib )))?"0":Number(remPriceAfDec(ee1 * spsftotcontrib )));
		$("input[name=ccEmplrContrbSps]").val(isNaN(Number(remPriceAfDec(er1 * spsftotcontrib )))?"0":Number(remPriceAfDec(er1 * spsftotcontrib ))); 
