...
//	if(!validateClientDets())return ;//If needed can uncommend
	childclearFlds();
...
//	$("#child_Dialog").find("input[id=txtFldDlgChildMode]").val("I");//instant save added line
...
//		$("#child_Dialog").find(".modal-footer").find("button:eq(0)").text("OK");
...
function valchildTbl(){  
	...
	if($RowCount > 0 ){ 
		$("#childParticularsTable tbody tr").each(function(){
			var $lastRow=$(this);
		if(!(validateFocusDhtmlFlds($lastRow.find("td:eq(2)").find('input:eq(0)'), CHILD_NAME))){valid=false;return false;};  
		if(!(validateFocusDhtmlFlds($lastRow.find("td:eq(3)").find('input:eq(0)'), CHILD_DOB))){valid=false;return false;};
		if(!(validateFocusDhtmlFlds($lastRow.find("td:eq(5)").find('select:eq(0)'), CHILD_RELATION))){valid=false;return false;};
