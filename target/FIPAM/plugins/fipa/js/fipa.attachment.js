...
function validateAttchDetails(){ 
		...
		if($RowCount > 0 ){ 
			...
			$("#fnaAttachments tbody tr").each(function(){
				var $lastRow=$(this); 
				if(!(validateFocusDhtmlFlds($lastRow.find("td:eq(2)").find('select:eq(0)'), ATTACH_CATG))) {valid=false;return false;};
				if(!(validateFocusDhtmlFlds($lastRow.find("td:eq(3)").find('select:eq(0)'), ATTACH_TITLE))) {valid=false;return false;};
				if($lastRow.find("td:eq(4)").find('a').length == 0){
					if(!(validateFocusDhtmlFlds($lastRow.find("td:eq(4)").find('input:eq(0)'), ATTACH_DOC))){valid=false;return false;}; 
				}
			});
