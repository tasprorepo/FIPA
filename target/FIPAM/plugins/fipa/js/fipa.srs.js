...
if(dataset == null){	 //instant save added line
	instantCashSRSSave($lastRow,INS_MODE);		
}
...
//	var $lastRow = $("#srsTable tbody tr:last");	
	...
	if($RowCount > 0 ){ 
		$("#srsTable tbody tr").each(function(){
			var $lastRow=$(this);
		if(!(validateFocusDhtmlFlds($lastRow.find("td:eq(2)").find('select:eq(0)'), SRS_CLASSFY))) {valid=false;return false;};
		if(!(validateFocusDhtmlFlds($lastRow.find("td:eq(3)").find('input:eq(0)'), SRS_DESC))) {valid=false;return false;}; 
		if(!(validateFocusDhtmlFlds($lastRow.find("td:eq(4)").find('select:eq(0)'), SRS_FREQ))){valid=false;return false;};
		if($lastRow.find("td:eq(2)").find('select:eq(0)').val() == "Annual Addition"){
			if(!(validateFocusDhtmlFlds($lastRow.find("td:eq(7)").find('input:eq(0)'), SRS_PERTO))){valid=false;return false;};
		} 
		});
	}  
	 return valid;
...
function cshSRSfilldlgval($lastRow){
	...
	//$('#Srs_Dialog #txtFldDlgSRSMode').val($lastRow.find("td:eq(0)").find('input:eq(0)').val());//instant save added line
	  $('#Srs_Dialog #txtFldDlgSRSId').val($lastRow.find("td:eq(0)").find('input:eq(1)').val());
	  $('#Srs_Dialog #txtFldDlgSRSRefId').val($lastRow.find("td:eq(0)").find('input:eq(2)').val());
...
function setSRSWithdrwDesc(classfyId,descId){
	if(classfyId.val() == "Annual Withdrawals"){	 
		if(!chkForWithdrawal(classfyId))return;
		descId.val("Cash Out");
	}else{ 
		descId.val("");
	}
...
function chkForWithdrawal(elmid){
	...
	if(!flg){
		applyErrorToastrAlert("Classification cannot be Withdrawal again since there is already a Annual Withdrawal record exist",elmid);
		elmid.val("");
	}
	return flg;
}
